...
class TestStripeInvoices:
    @pytest.fixture(scope="class")
    def unpaid_invoice_id(self, admin_session):
        r = admin_session.get(f"{BASE_URL}/api/finance/invoices?limit=50", timeout=30)
        assert r.status_code == 200, f"list invoices failed: {r.status_code} {r.text[:200]}"
        invoices = r.json()
        # filter unpaid + not the smoke-test one + amount > 0
        skip_id = "0ffcc027-f8f3-415e-a164-eebfae4f55a3"
        candidates = [
            inv for inv in invoices
            if inv.get("status") != "paid"
            and inv.get("id") != skip_id
            and float(inv.get("amount") or 0) > 0
        ]
        if not candidates:
            pytest.skip("No unpaid invoices available to test Stripe checkout")
        return candidates[0]["id"]

    def test_create_checkout_session(self, admin_session, unpaid_invoice_id):
        r = admin_session.post(
            f"{BASE_URL}/api/finance/invoices/{unpaid_invoice_id}/stripe-checkout",
        ...
        # Store for later assertions on activity log
        pytest._iter23_session_id = data["session_id"]
        pytest._iter23_invoice_id = unpaid_invoice_id

    def test_finance_activity_log_has_event(self, admin_session):
        invoice_id = getattr(pytest, "_iter23_invoice_id", None)
        if not invoice_id:
            pytest.skip("No checkout created in prior test")
        r = admin_session.get(
            f"{BASE_URL}/api/finance/activity?invoice_id={invoice_id}",
            timeout=30,
        )
        assert r.status_code == 200, f"activity log failed: {r.status_code} {r.text[:200]}"
        events = r.json()
        assert isinstance(events, list)
        ev_types = [e.get("event") for e in events]
        assert "stripe_checkout_started" in ev_types, f"Activity events: {ev_types}"
        # Validate structure of latest event
        started = next(e for e in events if e.get("event") == "stripe_checkout_started")
        assert started["invoice_id"] == invoice_id
        assert started.get("amount", 0) > 0
        assert started.get("currency")
        assert started.get("by") == ADMIN_EMAIL

    def test_public_invoice_endpoint(self, unpaid_invoice_id):
        # No auth — should still return minimal info
        r = requests.get(f"{BASE_URL}/api/public/invoice/{unpaid_invoice_id}", timeout=30)
        assert r.status_code == 200, f"public invoice failed: {r.status_code} {r.text[:200]}"
        data = r.json()
        assert data.get("id") == unpaid_invoice_id
        assert "invoice_no" in data
        assert "amount" in data
        assert "currency" in data
        assert "status" in data
        assert "project_name" in data
        # stripe_pay_url should now be set since checkout was created above
        assert data.get("stripe_pay_url", "").startswith("https://checkout.stripe.com/")

    def test_webhook_invalid_signature_returns_400(self):
        # Empty/garbage signature must yield 400
        r = requests.post(
            f"{BASE_URL}/api/webhook/stripe",
            data=b'{"id":"evt_test","type":"checkout.session.completed"}',
            headers={"Stripe-Signature": "t=1,v1=invalidsig", "Content-Type": "application/json"},
            timeout=30,
        )
        assert r.status_code == 400, f"Expected 400, got {r.status_code} {r.text[:200]}"

    def test_paid_invoice_rejects_checkout(self, admin_session):
        # Look for a paid invoice
        r = admin_session.get(f"{BASE_URL}/api/finance/invoices?limit=100", timeout=30)
        assert r.status_code == 200
        invoices = r.json()
        paid = [inv for inv in invoices if inv.get("status") == "paid"]
        if not paid:
            pytest.skip("No paid invoices in DB to test 400 rejection")
        pid = paid[0]["id"]
        r2 = admin_session.post(
            f"{BASE_URL}/api/finance/invoices/{pid}/stripe-checkout",
            json={"origin_url": BASE_URL},
            timeout=30,
        )
        assert r2.status_code == 400
        body = r2.text.lower()
        assert "already paid" in body, f"Expected 'already paid' in {r2.text[:200]}"


# ============================================================
# 4. Regression: public invoice 404 + activity gating
# ============================================================
