...
class PayslipSignatoryIn(BaseModel):
    """Company-side signature applied once to ALL payslips."""
    name: str = ""
    designation: str = "Authorised Signatory"
...
# --- Sign-docs ---
class SignDocAttachment(BaseModel):
    name: str
...
class SignDocIn(BaseModel):
    ...
    attachments: Optional[List[SignDocAttachment]] = None


class SignActionIn(BaseModel):
    signed_name: str  # what the user types as their "signature"
    signature_data_url: Optional[str] = None  # drawn signature (data:image/png;base64,...)
    x: Optional[float] = None  # placement, % of page width (top-left of sig box)
    y: Optional[float] = None  # placement, % of page height
...
def _draw_payslip_pdf(payslip: Dict, employer_address: str = "Projexino Solutions, India", signatory: Optional[Dict] = None) -> bytes:
    ...
    # ─────────────────────────────────────────────────────────────
    # Header band — soft warm background, logo PNG sits directly on it
    # ─────────────────────────────────────────────────────────────
    ...
    # orange accent bar under the band
    c.setFillColor(colors.HexColor("#F97316"))
    c.rect(0, H - 35.5 * mm, W, 1.5 * mm, stroke=0, fill=1)
    # subtle decorative circles (right side, behind text)
    c.setFillColor(colors.HexColor("#FDEBD8"))
    c.circle(W - 8 * mm, H - 4 * mm, 16 * mm, fill=1, stroke=0)
    c.setFillColor(colors.HexColor("#FBDCBB"))
    c.circle(W - 3 * mm, H - 30 * mm, 9 * mm, fill=1, stroke=0)

    # Logo — drawn straight on the light band (transparent PNG)
    if _draw_projexino_logo:
        _draw_projexino_logo(c, x_left=16 * mm, y_bottom=H - 25 * mm, height_mm=13 * mm)
    else:
        c.setFillColor(colors.HexColor("#0F2042"))
        c.setFont(bold_font, 16)
        c.drawString(16 * mm, H - 20 * mm, "PROJEXINO")
    c.setFillColor(colors.HexColor("#475569"))
    c.setFont(bold_font, 7.5)
    c.drawString(16 * mm, H - 30 * mm, "ENGINEERING STUDIOS  ·  DEDICATED TEAMS  ·  AI WORKFLOWS")

    # Right column — payslip metadata (navy on light)
    c.setFillColor(colors.HexColor("#F97316"))
    c.setFont(bold_font, 8)
    c.drawRightString(W - 18 * mm, H - 12 * mm, "// PAYSLIP")
    c.setFillColor(colors.HexColor("#0F2042"))
    c.setFont(bold_font, 16)
    c.drawRightString(W - 18 * mm, H - 19 * mm, f"PAYSLIP · {payslip['month']}")
    c.setFillColor(colors.HexColor("#64748B"))
    c.setFont(base_font, 9)
    c.drawRightString(W - 18 * mm, H - 24.5 * mm, f"Slip No: {payslip.get('slip_no','')}")
    c.drawRightString(W - 18 * mm, H - 29 * mm, f"Generated: {payslip.get('generated_at','')[:10]}")
    ...
    left_pairs = [
        ...
        ("Department",     (emp.get("department") or "Engineering").title()),
    ...
    # Authorised signatory only (configured once in payslip settings, applied to all payslips)
    sig_y = 32 * mm
    signatory = signatory or {}
    _durl = signatory.get("signature_data_url") or ""
    if _durl.startswith("data:"):
        try:
            import base64 as _b64
            from reportlab.lib.utils import ImageReader
            _img = ImageReader(io.BytesIO(_b64.b64decode(_durl.split(",", 1)[1])))
            _iw, _ih = _img.getSize()
            _w = 42 * mm
            _h = min(_w * _ih / _iw, 16 * mm)
            c.drawImage(_img, W - 80 * mm, sig_y + 1 * mm, width=_w, height=_h,
    ...
    c.drawString(W - 80 * mm, sig_y - 4 * mm, (signatory.get("name") or "").strip() or "Authorised Signatory")
    c.setFillColor(colors.HexColor("#64748B"))
    c.setFont(base_font, 7.5)
    c.drawString(W - 80 * mm, sig_y - 7.5 * mm, f"{signatory.get('designation') or 'Authorised Signatory'} \u00b7 Projexino Solutions")
...
def register_hr_module(api: APIRouter, db, get_current_user):
    ...
    @api.put("/hr/payslip-config/signatory")
    async def update_signatory(payload: PayslipSignatoryIn, user=Depends(get_current_user)):
        _priv(user)
        await db.hr_settings.update_one(
            {"id": "payslip"},
            {"$set": {"id": "payslip", "signatory": payload.model_dump(), "updated_at": _now_iso()}},
            upsert=True,
        )
        return await db.hr_settings.find_one({"id": "payslip"}, {"_id": 0})
    ...
    async def _resolve_employee(employee_id: str):
        ...
        # Fallback: HR onboarding record carries base_salary/designation/department
        ob = None
        if not tm or not tm.get("salary"):
            ob = await db.onboarding_records.find_one(
                {"email": u.get("email", "")},
                {"_id": 0, "base_salary": 1, "designation": 1, "department": 1})
        return {
            "id": u["id"],
            "name": u.get("name", ""),
            "email": u.get("email", ""),
            "role": u.get("role", ""),
            "designation": ((tm or {}).get("designation") or (tm or {}).get("role")
                            or (ob or {}).get("designation") or u.get("role", "")),
            "salary": (tm or {}).get("salary") or (ob or {}).get("base_salary") or 0.0,
            "department": (tm or {}).get("department") or (ob or {}).get("department", ""),
        }
    ...
    async def generate_payslip(payload: GeneratePayslipIn, user=Depends(get_current_user)):
        ...
        if payload.auto_email:
            try:
                pdf_bytes = _draw_payslip_pdf(doc, employer_address=(cfg.get("schedule") or {}).get("employer_address", "Projexino Solutions, India"), signatory=cfg.get("signatory"))
    ...
    async def list_payslips(employee_id: Optional[str] = None, month: Optional[str] = None,
        ...
        # Strict privacy: a non-priv user cannot query someone ELSE's payslips.
        # Querying without `employee_id` scopes to the caller's own list (used by
        # the "My Payslips" tab on the employee profile).
        if user.get("role") not in PRIV_VIEW_ROLES:
            if employee_id and employee_id != user["id"]:
                raise HTTPException(403, "Not authorised — payroll data is private")
            employee_id = user["id"]
        q: Dict[str, Any] = {}
        if employee_id:
    ...
    async def payslip_pdf(slip_id: str, user=Depends(get_current_user)):
        ...
        pdf = _draw_payslip_pdf(doc, employer_address=addr, signatory=cfg.get("signatory"))
    ...
    async def list_sign_docs(user=Depends(get_current_user)):
        # Don't ship binary blobs in list view — strip content_base64 from attachments
        cur = db.hr_sign_docs.find({}, {"_id": 0, "attachments.content_base64": 0}).sort("created_at", -1)
    ...
    async def create_sign_doc(payload: SignDocIn, user=Depends(get_current_user)):
        ...
        for a in payload.attachments or []:
            if a.size > MAX:
                raise HTTPException(400, f"'{a.name}' exceeds 10MB limit")
            attachments.append({
                "id": str(uuid.uuid4()),
                "name": a.name[:200],
        ...
        out = {**doc, "attachments": [{k: v for k, v in a.items() if k != "content_base64"} for a in attachments]}
        out.pop("_id", None)
        return out

    @api.get("/hr/sign-docs/{doc_id}/attachments/{att_id}/download")
    async def download_sign_doc_attachment(doc_id: str, att_id: str, user=Depends(get_current_user)):
        d = await db.hr_sign_docs.find_one({"id": doc_id}, {"_id": 0})
        if not d:
            raise HTTPException(404, "Doc not found")
        att = next((a for a in (d.get("attachments") or []) if a.get("id") == att_id), None)
        if not att:
    ...
    async def sign_doc(doc_id: str, payload: SignActionIn, user=Depends(get_current_user)):
        ...
        sig = {
            ...
            "signature_data_url": payload.signature_data_url or "",
    ...
    @api.post("/hr/sign-docs/{doc_id}/sign-link")
    async def sign_doc_link(doc_id: str, user=Depends(get_current_user)):
        """Personal tokenized QR link — sign this doc from any device, no login."""
        d = await db.hr_sign_docs.find_one({"id": doc_id}, {"_id": 0, "id": 1})
        if not d:
            raise HTTPException(404, "Doc not found")
        existing = await db.hr_sign_tokens.find_one({"doc_id": doc_id, "user_id": user["id"]}, {"_id": 0})
        if existing:
            return {"token": existing["token"]}
        token = uuid.uuid4().hex[:12]
        await db.hr_sign_tokens.insert_one({
            "token": token, "doc_id": doc_id, "user_id": user["id"],
            "name": user.get("name", ""), "email": user.get("email", ""),
            "role": user.get("role", ""), "created_at": _now_iso(),
        })
        return {"token": token}

    @api.get("/public/doc-sign/{token}")
    async def public_doc_sign_info(token: str):
        t = await db.hr_sign_tokens.find_one({"token": token}, {"_id": 0})
        if not t:
            raise HTTPException(404, "Invalid or expired sign link")
        d = await db.hr_sign_docs.find_one({"id": t["doc_id"]}, {"_id": 0, "attachments.content_base64": 0})
        if not d:
            raise HTTPException(404, "Document no longer exists")
        already = any(s.get("user_id") == t["user_id"] for s in d.get("signatures") or [])
        return {"doc_name": d.get("name"), "body_html": d.get("body_html", ""),
                "user_name": t.get("name"), "already_signed": already}

    @api.post("/public/doc-sign/{token}")
    async def public_doc_sign(token: str, payload: Dict[str, Any] = Body(...)):
        t = await db.hr_sign_tokens.find_one({"token": token}, {"_id": 0})
        if not t:
            raise HTTPException(404, "Invalid or expired sign link")
        d = await db.hr_sign_docs.find_one({"id": t["doc_id"]}, {"_id": 0})
        if not d:
            raise HTTPException(404, "Document no longer exists")
        if any(s.get("user_id") == t["user_id"] for s in d.get("signatures") or []):
            raise HTTPException(400, "You've already signed this document")
        ...
        sig = {
            "id": str(uuid.uuid4()), "user_id": t["user_id"], "name": t.get("name", ""),
            "email": t.get("email", ""), "role": t.get("role", ""),
            "typed_signature": signed_name,
            "signature_data_url": (payload.get("signature_data_url") or "")[:400000],
            "x": _f(payload.get("x")), "y": _f(payload.get("y")),
            "signed_at": _now_iso(), "via": "qr",
        }
        await db.hr_sign_docs.update_one({"id": t["doc_id"]}, {"$push": {"signatures": sig}, "$set": {"updated_at": _now_iso()}})
        return {"ok": True}

    # ---- Sign-doc PDF preview + drag-drop signature stamping ----
    def _stamp_sigs_on_pdf(base_pdf: bytes, sigs: List[Dict]) -> bytes:
        """Overlay placed signatures (x/y = % of page, top-left of box) onto page 1."""
        import base64 as _b
        from pypdf import PdfReader, PdfWriter
        from reportlab.pdfgen import canvas as rl_canvas
        from reportlab.lib.utils import ImageReader
        reader = PdfReader(io.BytesIO(base_pdf))
        pw = float(reader.pages[0].mediabox.width); ph = float(reader.pages[0].mediabox.height)
        buf = io.BytesIO()
        c = rl_canvas.Canvas(buf, pagesize=(pw, ph))
        for s in sigs:
            try:
                left = float(s["x"]) / 100.0 * pw; top = float(s["y"]) / 100.0 * ph
            except (TypeError, ValueError, KeyError):
                continue
            box_w = pw * 0.20
            img_h = 0.0
            durl = s.get("signature_data_url") or ""
            if durl.startswith("data:"):
                try:
                    img = ImageReader(io.BytesIO(_b.b64decode(durl.split(",", 1)[1])))
                    iw, ih = img.getSize()
                    img_h = min(box_w * ih / iw, 42.0)
                    c.drawImage(img, left, ph - top - img_h, width=box_w, height=img_h,
                                preserveAspectRatio=True, anchor="sw", mask="auto")
                except Exception:
                    img_h = 0.0
            base_y = ph - top - max(img_h, 16)
            c.setLineWidth(1); c.setStrokeColorRGB(0.06, 0.13, 0.26)
            c.line(left, base_y - 2, left + box_w, base_y - 2)
            c.setFont("Helvetica-Bold", 8); c.setFillColorRGB(0.06, 0.13, 0.26)
            c.drawString(left, base_y - 12, (s.get("typed_signature") or s.get("name") or "")[:44])
            c.setFont("Helvetica", 6.5); c.setFillColorRGB(0.42, 0.47, 0.55)
            c.drawString(left, base_y - 21, f"{(s.get('role') or '').replace('_', ' ')} · signed {(s.get('signed_at') or '')[:10]}"[:60])
        c.save(); buf.seek(0)
    ...
    def _sign_doc_pdf_bytes(d: Dict, hide_placed: bool = False) -> bytes:
        """Doc-to-sign as PDF: first PDF attachment if any, else branded HTML render.
        Placed signatures are stamped on page 1 unless hide_placed."""
        import base64 as _b
        sigs = d.get("signatures") or []
        placed = [] if hide_placed else [s for s in sigs if s.get("x") is not None and s.get("y") is not None]
        att = next((a for a in (d.get("attachments") or [])
                    if "pdf" in (a.get("mime_type") or "").lower() or (a.get("name") or "").lower().endswith(".pdf")), None)
        if att and att.get("content_base64"):
            base = _b.b64decode(att["content_base64"])
            return _stamp_sigs_on_pdf(base, placed) if placed else base
        # HTML fallback — branded page from body_html
        from weasyprint import HTML
        from extensions import logo_data_uri
        logo = logo_data_uri("")
        placed_html = ""
        for s in placed:
            left_mm = max(0.0, min(float(s["x"]) / 100.0 * 210 - 18, 170))
            top_mm = max(0.0, min(float(s["y"]) / 100.0 * 297 - 20, 250))
            img = s.get("signature_data_url") or ""
            placed_html += f"""<div style="position:absolute;left:{left_mm:.1f}mm;top:{top_mm:.1f}mm;width:42mm;">
              <div style="height:14mm;display:flex;align-items:flex-end;border-bottom:1.5px solid #0F2042;">{('<img src="' + img + '" style="max-height:13mm;max-width:100%;"/>') if img else '&nbsp;'}</div>
              <div style="font-weight:700;font-size:8pt;margin-top:2px;">{s.get('typed_signature') or s.get('name') or ''}</div>
              <div style="font-size:6.5pt;color:#64748B;">{(s.get('role') or '').replace('_', ' ')} · {(s.get('signed_at') or '')[:10]}</div>
            </div>"""
        flowing = [s for s in sigs if not (s.get("x") is not None and s.get("y") is not None)] if not hide_placed else []
        rows = "".join(
            f"<tr><td style='padding:4px 10px 4px 0;font-weight:700;'>{s.get('typed_signature') or s.get('name') or ''}</td>"
            f"<td style='padding:4px 10px 4px 0;color:#64748B;'>{(s.get('role') or '').replace('_', ' ')}</td>"
            f"<td style='padding:4px 0;color:#64748B;'>{(s.get('signed_at') or '')[:10]}</td></tr>" for s in flowing)
        signed_table = f"""<div style="margin-top:36px;border-top:1px solid #E2E8F0;padding-top:10px;">
          <div style="font-size:8pt;letter-spacing:.2em;text-transform:uppercase;color:#F97316;font-weight:700;">Signatures</div>
          <table style="font-size:9pt;margin-top:6px;">{rows}</table></div>""" if rows else ""
        html = f"""<!doctype html><html><head><meta charset="utf-8"><style>
        @page {{ size:A4; margin:20mm 18mm 22mm 18mm;
          @bottom-left {{ content:"Projexino Solutions Pvt Ltd"; font-size:7.5pt; color:#94A3B8; }}
          @bottom-right {{ content:"Page " counter(page) " of " counter(pages); font-size:7.5pt; color:#94A3B8; }} }}
        body {{ font-family:'Helvetica','Arial',sans-serif; color:#0F2042; font-size:10.5pt; line-height:1.65; }}
        </style></head><body>
        <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #F97316;padding-bottom:10px;margin-bottom:18px;">
          <img src="{logo}" style="height:40px;width:auto;" />
          <div style="text-align:right;font-size:8pt;color:#64748B;">
            <div style="font-weight:700;color:#0F2042;">Projexino Solutions Pvt Ltd</div>
            <div>REF · SD-{(d.get('id') or '')[:8].upper()}</div>
          </div>
        </div>
        <div style="font-size:8.5pt;letter-spacing:.25em;text-transform:uppercase;color:#F97316;font-weight:700;">// Document to sign</div>
        <h1 style="font-size:15pt;margin:6px 0 14px;">{d.get('name', '')}</h1>
        <div style="text-align:justify;">{d.get('body_html') or '<p style="color:#94A3B8;">No document body — see attachments.</p>'}</div>
        {signed_table}
        {placed_html}
        </body></html>"""
        return HTML(string=html).write_pdf()

    @api.get("/hr/sign-docs/{doc_id}/pdf")
    async def sign_doc_pdf(doc_id: str, hide_placed: bool = False, user=Depends(get_current_user)):
        d = await db.hr_sign_docs.find_one({"id": doc_id}, {"_id": 0})
        if not d:
            raise HTTPException(404, "Doc not found")
        pdf = _sign_doc_pdf_bytes(d, hide_placed=hide_placed)
        return StreamingResponse(io.BytesIO(pdf), media_type="application/pdf",
                                 headers={"Content-Disposition": f'inline; filename="{(d.get("name") or "document")[:60]}.pdf"'})

    @api.get("/public/doc-sign/{token}/pdf")
    async def public_sign_doc_pdf(token: str, hide_placed: bool = False):
        t = await db.hr_sign_tokens.find_one({"token": token}, {"_id": 0})
        if not t or not t.get("doc_id"):
            raise HTTPException(404, "Invalid or expired sign link")
        d = await db.hr_sign_docs.find_one({"id": t["doc_id"]}, {"_id": 0})
        if not d:
            raise HTTPException(404, "Document no longer exists")
    ...
    @api.patch("/hr/sign-docs/{doc_id}/signatures")
    async def place_sign_doc_signatures(doc_id: str, payload: Dict[str, Any] = Body(...), user=Depends(get_current_user)):
        """Save drag-drop positions. Signers may move their own signature; HR/admin any."""
        d = await db.hr_sign_docs.find_one({"id": doc_id}, {"_id": 0})
        if not d:
            raise HTTPException(404, "Doc not found")
        positions = {p.get("id"): p for p in (payload.get("positions") or []) if p.get("id")}
        is_priv = user.get("role") in PRIV_ROLES
        sigs = d.get("signatures") or []
        changed = False
        for s in sigs:
            p = positions.get(s.get("id"))
            if not p:
                continue
            if not is_priv and s.get("user_id") != user["id"]:
                raise HTTPException(403, "You can only position your own signature")
            def _f(v):
                try: return round(float(v), 1)
                except (TypeError, ValueError): return None
            s["x"], s["y"] = _f(p.get("x")), _f(p.get("y"))
            changed = True
        if changed:
            await db.hr_sign_docs.update_one({"id": doc_id}, {"$set": {"signatures": sigs, "updated_at": _now_iso()}})
    ...
    async def audit_report(year: int, user=Depends(get_current_user)):
        ...
        # Receipts pushed to audit from Finance
        for k in month_buckets:
            month_buckets[k]["receipt_income"] = 0.0
        rec_total, rec_count = 0.0, 0
        async for r in db.audit_records.find(
                {"record_date": {"$gte": start[:10], "$lt": end[:10]}}, {"_id": 0}):
            amt = float(r.get("amount") or 0)
            m = (r.get("record_date") or "")[:7]
            if m in month_buckets:
                month_buckets[m]["receipt_income"] += amt
            rec_total += amt
            rec_count += 1
        return {
            "year": year,
            "totals": {
                ...
                "receipts_inr": round(rec_total, 2), "audit_receipts": rec_count,
            },
            "monthly": [{"month": k, **v} for k, v in sorted(month_buckets.items())],
        }

    # ============== AUDIT RECORDS (receipts pushed from Finance) ==============
    @api.get("/hr/audit-records")
    async def list_audit_records(year: Optional[int] = None, ids_only: bool = False, user=Depends(get_current_user)):
        _priv(user, PRIV_VIEW_ROLES)
        if ids_only:
            cur = db.audit_records.find({}, {"_id": 0, "receipt_id": 1})
            return {"receipt_ids": [r["receipt_id"] async for r in cur]}
        q: Dict = {}
        if year:
            q["record_date"] = {"$gte": f"{year}-01-01", "$lt": f"{year + 1}-01-01"}
        return await db.audit_records.find(q, {"_id": 0}).sort("record_date", -1).to_list(2000)

    @api.post("/hr/audit-records")
    async def add_audit_record(payload: Dict, user=Depends(get_current_user)):
        _priv(user)
        receipt_id = (payload or {}).get("receipt_id")
        if not receipt_id:
            raise HTTPException(400, "receipt_id required")
        rec = await db.receipts.find_one({"id": receipt_id}, {"_id": 0})
        if not rec:
            raise HTTPException(404, "Receipt not found")
        if await db.audit_records.find_one({"receipt_id": receipt_id}):
            raise HTTPException(400, "Receipt is already in the audit list")
        doc = {
            "id": str(uuid.uuid4()), "source": "receipt", "receipt_id": receipt_id,
            "receipt_no": rec.get("receipt_no"), "invoice_no": rec.get("invoice_no"),
            "client_name": rec.get("client_name"), "project_name": rec.get("project_name"),
            "amount": float(rec.get("amount") or 0), "currency": rec.get("currency", "INR"),
            "method": rec.get("method", ""),
            "record_date": (rec.get("approved_at") or _now_iso())[:10],
            "added_by": {"id": user["id"], "name": user.get("name", "")},
            "created_at": _now_iso(),
        }
        await db.audit_records.insert_one(dict(doc))
        return doc

    @api.delete("/hr/audit-records/{rid}")
    async def delete_audit_record(rid: str, user=Depends(get_current_user)):
        _priv(user)
        res = await db.audit_records.delete_one({"id": rid})
