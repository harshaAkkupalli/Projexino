...
# Strict lifecycle graph — what each status can move to. Used by PATCH /tasks/{id}.
# Empty string keys map both legacy "open" and "todo".
TASK_TRANSITIONS: Dict[str, List[str]] = {
    "todo": ["in_progress", "blocked"],
    "in_progress": ["todo", "blocked", "review", "done"],
    "blocked": ["todo", "in_progress"],
    "review": ["in_progress", "done"],   # done is allowed only via /approve
    "done": ["in_progress", "review"],   # re-opening
}

# Roles that may approve/reject and complete tasks directly. Anyone else who
# tries to set status="done" gets auto-routed to "review" with is_locked=True.
APPROVAL_ROLES = ("super_admin", "admin", "manager", "hr")


class TaskDependency(BaseModel):
    type: Literal["blocks"] = "blocks"   # only "blocks" supported today (parent→child)
    task_id: str
...
class Task(BaseModel):
    ...
    created_at: str
    updated_at: str
    # Iter 35 — strict lifecycle additions
    dependencies: List[TaskDependency] = []   # parent task ids that block THIS task
    is_locked: bool = False                   # True while awaiting manager approval
    submitted_for_review_by: str = ""         # uid of the user who moved to "review"
    submitted_for_review_at: str = ""
    approved_by: str = ""                     # uid of the approver
    approved_at: str = ""
    rejected_reason: str = ""                 # last rejection note (cleared on re-approve)
    rejected_at: str = ""
...
async def create_task(payload: TaskCreate, user=Depends(get_current_user)):
    ...
    # Iter 37 — OOO guard: refuse to assign a task whose due_date falls on a day
    # the assignee has approved leave. Only enforced when both are present.
    if doc.get("assignee_id") and doc.get("due_date"):
        try:
            from leave_module import is_employee_ooo_on
            ooo = await is_employee_ooo_on(db, doc["assignee_id"], doc["due_date"])
            if ooo:
                raise HTTPException(
                    409,
                    f"Cannot assign — {assignee_user.get('name') or assignee_user['email'] if assignee_user else 'assignee'} is on approved {ooo.get('leave_type','leave').upper()} on {doc['due_date']}.",
                )
        except HTTPException:
            raise
        except Exception:
            logger.exception("OOO guard failed; continuing")
...
# ----- Helpers for lifecycle enforcement -----
def _can_approve(role: str) -> bool:
    return role in APPROVAL_ROLES


async def _is_task_stakeholder(task: dict, user: dict) -> bool:
    """Manager, owner, RM, or admin/super_admin can approve/reject a task."""
    if user.get("role") in APPROVAL_ROLES:
        return True
    uid = user["id"]
    return (
        task.get("owner_id") == uid
        or task.get("reporting_manager_id") == uid
    )


async def _on_task_completed(db_, task: dict, actor: dict) -> List[str]:
    """Hand-off Trigger: when a task becomes 'done', find every other task
    that lists this task in its dependencies (type=blocks) and is currently
    blocked, then flip them to 'todo' and notify their assignees.
    ...
    try:
        from notif_engine import notify
        cur = db_.tasks.find(
            {"dependencies.task_id": task["id"], "status": "blocked"},
            {"_id": 0},
        )
        async for child in cur:
            await db_.tasks.update_one(
                {"id": child["id"]},
                {"$set": {"status": "todo", "updated_at": datetime.now(timezone.utc).isoformat()}},
            )
            unblocked.append(child["id"])
            # Notify assignee + RM (if any), excluding the actor
            recipients = {child.get("assignee_id"), child.get("reporting_manager_id")}
            recipients.discard("")
            recipients.discard(actor["id"])
            for rid in recipients:
                rdoc = await db_.users.find_one({"id": rid}, {"_id": 0, "id": 1, "email": 1, "name": 1})
                if not rdoc:
                    continue
                await notify(
                    db_,
                    ...
                    user_id=rdoc["id"],
                    user_email=rdoc.get("email", ""),
                    title=f"Unblocked: {child['title']}",
                    message=f"Task '{task['title']}' just completed — '{child['title']}' is now ready for action.",
                    link=f"/app/tasks/{child['id']}",
                    variables={"name": rdoc.get("name", ""), "parent_title": task["title"], "task_title": child["title"]},
                    triggered_by={"name": actor.get("name", ""), "email": actor.get("email", "")},
                )
    except Exception:
        logger.exception("on_task_completed hand-off failed")
    return unblocked
...
async def update_task(task_id: str, payload: TaskUpdate, user=Depends(get_current_user)):
    ...
    Strict status lifecycle (iter 35):
      todo ↔ in_progress, todo → blocked, blocked → todo/in_progress,
      in_progress → review/done, review → done (approval required), review → in_progress.

    Interns / team_members CANNOT transition a task directly to 'done'. Any
    such attempt is auto-routed to 'review' and the task is locked
    (is_locked=True) until a manager calls /tasks/{id}/approve or /reject.
    """
    uid = user["id"]
    ...
    auto_routed_to_review = False
    completed_after_update = False
    for k, v in raw.items():
        if v is None and k != "dependencies":
            continue
        updates[k] = v

    # ---- LIFECYCLE GUARD ----
    if "status" in updates:
        current = existing.get("status") or "todo"
        target = updates["status"]
        # Auto-route low-privilege users away from "done".
        if target == "done" and not _can_approve(role) and existing.get("assignee_id") == uid:
            target = "review"
            auto_routed_to_review = True
        # If the task is locked (awaiting approval), low-privilege users may not flip status.
        if existing.get("is_locked") and not _can_approve(role):
            raise HTTPException(
                status_code=409,
                detail="Task is locked pending manager review. Use /approve or /reject as a manager.",
            )
        allowed = TASK_TRANSITIONS.get(current, [])
        if target != current and target not in allowed and not _can_approve(role):
            raise HTTPException(
                status_code=400,
                detail=f"Invalid status transition '{current}' → '{updates['status']}'. Allowed: {allowed}",
            )
        updates["status"] = target

        if target == "review":
            updates["is_locked"] = True
            updates["submitted_for_review_by"] = uid
            updates["submitted_for_review_at"] = datetime.now(timezone.utc).isoformat()
        if target == "done":
    ...
    # Dependencies — serialise the Pydantic models if present
    if "dependencies" in updates and updates["dependencies"] is not None:
        deps = updates["dependencies"]
        updates["dependencies"] = [d.model_dump() if hasattr(d, "model_dump") else d for d in deps]
    ...
    try:
        ...
        if "status" in updates and updates["status"] != existing.get("status"):
            ...
            # Stakeholder broadcast (owner / RM / assignee minus actor)
            recipients = set()
            for key in ("owner_id", "reporting_manager_id", "assignee_id"):
                rid = existing.get(key)
                if rid and rid != uid:
                    recipients.add(rid)

            # If the task moved into REVIEW, an explicit approval prompt is needed
            review_recipients = set()
            if new_status == "review":
                for key in ("reporting_manager_id", "owner_id"):
                    rid = existing.get(key)
                    if rid and rid != uid:
                        review_recipients.add(rid)
                # Also broadcast to any super_admin if no specific manager is set
                if not review_recipients:
                    sa_cur = db.users.find({"role": "super_admin"}, {"_id": 0, "id": 1, "email": 1, "name": 1})
                    async for sa in sa_cur:
                        if sa["id"] != uid:
                            review_recipients.add(sa["id"])

            for rid in recipients:
                rdoc = await db.users.find_one({"id": rid}, {"_id": 0, "id": 1, "email": 1, "name": 1})
                if not rdoc:
                    continue
                title_prefix = "🛎️ Approval needed" if (rid in review_recipients) else "Task update"
                await notify(
                    db,
                    event="task_review_requested" if new_status == "review" and rid in review_recipients else "task_status_changed",
                    user_id=rdoc["id"],
                    user_email=rdoc.get("email", ""),
                    title=f"{title_prefix}: {existing.get('title','')}",
                    message=f"{actor_name} moved this task to '{new_status.replace('_',' ')}'.",
                    link=f"/app/tasks/{task_id}",
                    variables={
                        "name": rdoc.get("name", ""),
                        ...
                        "requires_approval": (rid in review_recipients),
                    },
                    triggered_by={"name": user.get("name", ""), "email": user.get("email", "")},
                )
    except Exception:
        logger.exception("task_status_changed notify failed")

    # Hand-off Trigger if we transitioned to "done"
    if completed_after_update:
        await _on_task_completed(db, merged, user)

    # Pass auto-route flag in header so the frontend can show a toast
    if auto_routed_to_review:
        merged["_auto_routed_to_review"] = True
    return Task(**merged)


@api.post("/tasks/{task_id}/approve", response_model=Task)
async def approve_task(task_id: str, user=Depends(get_current_user)):
    """Manager/Admin approves a task in 'review' status → moves it to 'done',
    unlocks, fires task_completed notifications, and cascades dependency hand-off."""
    existing = await db.tasks.find_one({"id": task_id}, {"_id": 0})
    if not existing:
        raise HTTPException(status_code=404, detail="Task not found")
    if not await _is_task_stakeholder(existing, user):
        raise HTTPException(status_code=403, detail="Only the manager, owner, or admin can approve")
    if existing.get("status") != "review":
        raise HTTPException(status_code=400, detail=f"Cannot approve from status '{existing.get('status')}'. Task must be in 'review'.")
    now = datetime.now(timezone.utc).isoformat()
    updates = {
        "status": "done",
        "is_locked": False,
    ...
    # Notify assignee + owner that the task is approved/closed.
    try:
        from notif_engine import notify
        actor_name = user.get("name") or user.get("email") or "Your manager"
        recipients = set()
        for key in ("assignee_id", "owner_id", "reporting_manager_id"):
            rid = existing.get(key)
        ...
        for rid in recipients:
            ...
            if not rdoc:
                continue
            await notify(
                db, event="task_approved",
                user_id=rdoc["id"], user_email=rdoc.get("email", ""),
                title=f"✅ Approved & closed: {existing['title']}",
                message=f"{actor_name} approved your task. Great work!",
                link=f"/app/tasks/{task_id}",
                variables={"name": rdoc.get("name", ""), "task_title": existing["title"], "actor_name": actor_name},
                triggered_by={"name": user.get("name", ""), "email": user.get("email", "")},
            )
    except Exception:
        logger.exception("approve notify failed")

    # Cascade — flip any blocked-by-this children
    await _on_task_completed(db, merged, user)
    return Task(**merged)
...
@api.post("/tasks/{task_id}/reject", response_model=Task)
async def reject_task(task_id: str, payload: RejectIn, user=Depends(get_current_user)):
    """Manager/Admin rejects a 'review' task with feedback → back to 'in_progress'
    + unlock + persist the rejection reason + notify assignee."""
    existing = await db.tasks.find_one({"id": task_id}, {"_id": 0})
    if not existing:
        raise HTTPException(status_code=404, detail="Task not found")
    if not await _is_task_stakeholder(existing, user):
        raise HTTPException(status_code=403, detail="Only the manager, owner, or admin can reject")
    if existing.get("status") != "review":
        raise HTTPException(status_code=400, detail=f"Cannot reject from status '{existing.get('status')}'. Task must be in 'review'.")
    now = datetime.now(timezone.utc).isoformat()
    updates = {
        "status": "in_progress",
        "is_locked": False,
    ...
    try:
        from notif_engine import notify
        actor_name = user.get("name") or user.get("email") or "Your manager"
        recipients = {existing.get("assignee_id")}
        recipients.discard("")
        ...
        for rid in recipients:
            ...
            if not rdoc:
                continue
            await notify(
                db, event="task_rejected",
                user_id=rdoc["id"], user_email=rdoc.get("email", ""),
                title=f"↩️ Changes requested: {existing['title']}",
                message=f"{actor_name} returned this task with notes: {payload.reason}",
                link=f"/app/tasks/{task_id}",
                variables={"name": rdoc.get("name", ""), "task_title": existing["title"], "reason": payload.reason, "actor_name": actor_name},
                triggered_by={"name": user.get("name", ""), "email": user.get("email", "")},
            )
    except Exception:
        logger.exception("reject notify failed")
    return Task(**merged)


@api.get("/tasks/approvals/queue", response_model=List[Task])
async def approvals_queue(user=Depends(get_current_user)):
    """Tasks awaiting review that the current user can approve/reject.

    A user qualifies if they're an admin/super_admin/manager/hr, OR if they
    are listed as the task's `owner_id` or `reporting_manager_id`.
    """
    role = user.get("role") or ""
    uid = user["id"]
    q: Dict[str, Any] = {"status": "review"}
    if role not in APPROVAL_ROLES:
        q["$or"] = [{"reporting_manager_id": uid}, {"owner_id": uid}]
    cur = db.tasks.find(q, {"_id": 0}).sort("submitted_for_review_at", -1)
    items = await cur.to_list(200)
    return [Task(**d) for d in items]


@api.post("/tasks/deadlines/run")
async def trigger_deadline_daemon(user=Depends(get_current_user)):
    """Admin-only manual trigger — useful for testing or post-deploy smoke."""
    if user.get("role") not in ("super_admin", "admin"):
        raise HTTPException(403, "Only admins can trigger the daemon")
    from deadline_daemon import fire_now
    return await fire_now(db)
...
async def startup():
    ...
    # Iter 34 — unified intern UX: ensure interns have tasks.edit + projects.view
    try:
        from rbac import ensure_intern_unified_uplift
        await ensure_intern_unified_uplift(db)
    except Exception:
        logger.exception("ensure_intern_unified_uplift failed")
    ...
    # Iter 35: 08:00 daily deadline daemon (Daily Target Plan + Slipped Schedule Alert)
    try:
        from deadline_daemon import deadline_scheduler_loop
        fire_hour = int(os.environ.get("DEADLINE_DAEMON_HOUR_UTC", "8"))
        _asyncio.create_task(deadline_scheduler_loop(db, fire_hour=fire_hour))
    except Exception:
        logger.exception("deadline_daemon launch failed")
    ...
    # Bank credit-alert watcher (every 3 min)
    try:
        from bank_alerts import bank_alert_loop
        _asyncio.create_task(bank_alert_loop(db, interval_seconds=180))
        logger.info("bank_alert_loop started")
    except Exception:
        logger.exception("bank_alert_loop start failed")

    # Outreach reply-sync poller (every 15 min)
    async def _outreach_reply_loop():
        while True:
            try:
                fn = getattr(db, "_outreach_sync_replies", None)
                if fn:
                    await fn()
            except Exception:
                logger.exception("outreach_reply_loop tick failed")
            await _asyncio.sleep(900)  # 15 minutes
    _asyncio.create_task(_outreach_reply_loop())

    # Testimonial reminder loop (every 6h)
    try:
        fn = getattr(db, "_testimonial_reminder_loop", None)
        if fn:
            _asyncio.create_task(fn())
    except Exception:
        logger.exception("testimonial_reminder_loop start failed")
...
from public_invoices import register_public_invoices
from hr_certificates import register_hr_certificates
from playbooks import register_playbooks
...
from newsletter import register_newsletter
from outreach import register_outreach
from leave_module import register_leave
from wxm import register_wxm
from onboarding import register_onboarding
from clients_module import register_clients
from digi_module import register as register_digi
from testimonials_module import register_testimonials
from contracts_module import register_contracts
from hr_letters_module import register_hr_letters
...
register_public_invoices(ext_router, db, get_current_user)
register_hr_certificates(ext_router, db, get_current_user)
register_playbooks(ext_router, db, get_current_user)
from bank_alerts import register_bank_alerts
register_bank_alerts(ext_router, db, get_current_user)
from account_deletion import register_account_deletion
register_account_deletion(ext_router, db, get_current_user)
...
register_newsletter(ext_router, db, get_current_user)
register_outreach(ext_router, db, get_current_user)
register_leave(ext_router, db, get_current_user)
register_wxm(ext_router, db, get_current_user)
register_onboarding(ext_router, db, get_current_user)
register_clients(ext_router, db, get_current_user)
register_digi(ext_router, db, get_current_user)
register_testimonials(ext_router, db, get_current_user)
register_contracts(ext_router, db, get_current_user)
register_hr_letters(ext_router, db, get_current_user)
from careers import register_careers
register_careers(ext_router, db, get_current_user)
from wa_outreach import register_wa_outreach
register_wa_outreach(ext_router, db, get_current_user)
from doc_studio import register_doc_studio
register_doc_studio(ext_router, db, get_current_user)
