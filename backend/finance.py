...
class InvoiceCreateIn(BaseModel):
    ...
    due_date: Optional[str] = ""
    issue_date: Optional[str] = ""              # YYYY-MM-DD; blank = today (supports future-dated invoices)
...
async def create_receipt_for_invoice(db, inv: dict, method: str, approved_by: str, note: str = "", receipt_date: str = "") -> dict:
    """Shared: allocate receipt no, insert receipt, link back onto the invoice."""
    now = (f"{receipt_date}T00:00:00+00:00" if (receipt_date or "").strip()
...
"""
    ...
    async def create_invoice(fid: str, payload: InvoiceCreateIn, request: Request, user=Depends(get_current_user)):
        ...
        invoice = {
            ...
            "status": "issued",
            "issued_at": (f"{payload.issue_date}T00:00:00+00:00" if (payload.issue_date or "").strip() else now),
    ...
    async def create_receipt_manual(payload: Dict[str, Any] = Body(default={}), user=Depends(get_current_user)):
        ...
        receipt_date = str(payload.get("receipt_date") or "").strip()
        now = (f"{receipt_date}T00:00:00+00:00" if receipt_date else datetime.now(timezone.utc).isoformat())
        if invoice_id:
            inv = await db.invoices.find_one({"id": invoice_id}, {"_id": 0, "pdf_base64": 0})
            if not inv:
                raise HTTPException(404, "Invoice not found")
            ...
            rec = await create_receipt_for_invoice(db, inv, method, user.get("email"), note, receipt_date)
            await db.invoices.update_one(
                {"id": invoice_id},
                {"$set": {"status": "paid", "paid_at": now, "payment_approved_by": user.get("email")}})
            if inv.get("finance_id"):
