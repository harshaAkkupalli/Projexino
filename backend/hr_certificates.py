...
class CertCreate(BaseModel):
    ...
    content: str
    content_format: Optional[str] = "text"  # text | html
...
# ---- HTML content support (whitelist → ReportLab intra-paragraph markup) ----
import re as _re

_INLINE_TAGS = ("b", "i", "u", "font", "br", "sup", "sub")


def _sanitize_inline_html(seg: str) -> str:
    seg = _re.sub(r"<\s*(script|style)[^>]*>.*?<\s*/\s*\1\s*>", "", seg, flags=_re.S | _re.I)
    seg = _re.sub(r"<\s*strong(\s[^>]*)?>", "<b>", seg, flags=_re.I)
    seg = _re.sub(r"</\s*strong\s*>", "</b>", seg, flags=_re.I)
    seg = _re.sub(r"<\s*em(\s[^>]*)?>", "<i>", seg, flags=_re.I)
    seg = _re.sub(r"</\s*em\s*>", "</i>", seg, flags=_re.I)

    def _keep(m):
        return m.group(0) if m.group(1).lower() in _INLINE_TAGS else ""

    seg = _re.sub(r"</?([a-zA-Z0-9]+)(\s[^>]*)?/?>", _keep, seg)
    return seg.strip()


def _html_blocks(html: str):
    """Split raw HTML into (is_bullet, inline_markup) blocks for ReportLab Paragraphs."""
    html = (html or "").replace("\r\n", "\n")
    html = _re.sub(r"</?\s*(ul|ol)[^>]*>", "\x00", html, flags=_re.I)
    html = _re.sub(r"<\s*li[^>]*>", "\x00\x01", html, flags=_re.I)
    html = _re.sub(r"</\s*li\s*>", "\x00", html, flags=_re.I)
    html = _re.sub(r"</?\s*(p|div|h[1-6]|section|article)(\s[^>]*)?>", "\x00", html, flags=_re.I)
    html = html.replace("\n\n", "\x00")
    blocks = []
    for chunk in html.split("\x00"):
        bullet = chunk.startswith("\x01")
        text = _sanitize_inline_html(chunk.lstrip("\x01"))
        if text:
...
def _build_certificate_pdf(cert: dict) -> bytes:
    ...
    # ===== CONTENT (HR-authored — plain text or whitelisted HTML) =====
    raw = (cert.get("content") or "").replace("\r\n", "\n").strip()
    if (cert.get("content_format") or "text") == "html":
        bullet_style = ParagraphStyle("bl", parent=body_style, leftIndent=14, bulletIndent=4)
        for bullet, markup in _html_blocks(raw):
            try:
                if bullet:
                    story.append(Paragraph(markup, bullet_style, bulletText="•"))
                else:
                    story.append(Paragraph(markup, body_style))
            except Exception:
                plain = _re.sub(r"<[^>]+>", "", markup).replace("&", "&amp;")
...
def register_hr_certificates(api: APIRouter, db, get_current_user):
    ...
    async def create_certificate(payload: CertCreate, user=Depends(get_current_user)):
        ...
        if payload.content_format not in ("text", "html"):
    ...
    async def update_certificate(cid: str, payload: CertUpdate, user=Depends(get_current_user)):
        ...
        if updates.get("cert_type") and updates["cert_type"] not in CERT_TITLES:
            raise HTTPException(400, "cert_type must be 'internship' or 'performance'")
        if updates.get("content_format") and updates["content_format"] not in ("text", "html"):
            raise HTTPException(400, "content_format must be 'text' or 'html'")
