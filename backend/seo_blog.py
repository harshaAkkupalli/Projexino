...
class BlogPostIn(BaseModel):
    ...
    # NEW — dynamic CTA at the bottom of the post (overrides the default
    # "Start your project" CTA on the public page).
    cta_label: Optional[str] = ""
    cta_link: Optional[str] = ""
    cta_heading: Optional[str] = ""
    cta_subheading: Optional[str] = ""
    # NEW — list of file attachments (PDFs / images / docs) that can be
    # referenced from inside the content_html via the marker
    # {{attachment:<id>}} so the public renderer turns it into an inline
    # download card. Each attachment carries its own title + filename so
    # the visitor sees a nice "📄 Download — <title>" block.
...
def register_seo_blog(api: APIRouter, db, get_current_user):
    ...
    async def admin_create(payload: BlogPostIn, user=Depends(_require_admin)):
        ...
        doc = {
            ...
            "cta_heading": payload.cta_heading or "",
            "cta_subheading": payload.cta_subheading or "",
    ...
    # ---------- BLOG ATTACHMENTS (PDFs / inline images) ----------
    @api.post("/admin/blog/upload")
    async def admin_blog_upload(
        payload: Dict[str, Any] = Body(...),
        user=Depends(_require_admin),
    ):
        """Upload a base64-encoded image OR PDF that will be referenced from
        a blog post. Returns an attachment doc with `id`, `url`, `title`,
        `filename`, `mime`, `size_kb` so the frontend can insert it inline.

        The actual bytes are stored in `blog_assets` and served back via
        `/api/blog/assets/{id}` (public).
        """
        import base64
        b64 = (payload.get("data_base64") or "").strip()
        if not b64:
        ...
        if b64.startswith("data:"):
            try:
                ...
                if not (payload.get("mime")) and "image/" in head:
                    payload["mime"] = head.split(";")[0].replace("data:", "")
            except Exception:
                pass
        try:
        ...
        mime = (payload.get("mime") or "application/octet-stream").lower()
        kind = "image" if mime.startswith("image/") else "pdf" if "pdf" in mime else "file"
        asset_id = uuid.uuid4().hex
        title = (payload.get("title") or payload.get("filename") or "Attachment").strip()
        filename = (payload.get("filename") or f"{title}.{kind}").strip()
        asset = {
            "id": asset_id,
            "title": title,
            "filename": filename,
        ...
        await db.blog_assets.insert_one(asset)
        return {
            "id": asset_id,
            "title": title,
            "filename": filename,
            ...
            "url": f"/api/blog/assets/{asset_id}",
        }

    @api.get("/blog/assets/{asset_id}")
    async def blog_asset_get(asset_id: str, download: int = 0):
        """Public download/preview endpoint for blog assets (images embedded
        inline, PDFs downloaded with attachment disposition)."""
        import base64
        asset = await db.blog_assets.find_one({"id": asset_id}, {"_id": 0})
        if not asset:
            raise HTTPException(404, "Asset not found")
        try:
            raw = base64.b64decode(asset.get("data_base64") or "")
        except Exception:
            raise HTTPException(500, "Corrupt asset")
        mime = asset.get("mime") or "application/octet-stream"
        filename = asset.get("filename") or f"projexino-{asset_id[:6]}"
        # PDFs default to attachment download; images stay inline so they
        # render in <img> tags.
        disposition = "attachment" if (download or asset.get("kind") == "pdf") else "inline"
