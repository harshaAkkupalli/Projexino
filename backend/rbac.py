...
MODULES = [
    ...
    # Iter 36 — previously missing, which is why permissions silently
    # failed for these two cards before. Now grantable via /rbac/matrix.
    "outreach", "newsletter",
    # Iter 37 — Leave / PTO subsystem
    "leave",
    # Iter 80 — Doc Studio (AI/manual SDD, Plan & SRS generator)
    "doc-studio",
]
...
DEFAULT_MATRIX: Dict[str, Dict[str, Dict[str, bool]]] = {
    ...
    "intern": {
        # Interns now share the same /app/* portal as Admin. The backend
        # already filters projects/tasks by assignment (only their assigned
        # rows show up). We grant view + edit on tasks (status change & start)
        # and view-only on projects so they can see what they're assigned to.
        m: {"view": True, "create": False, "edit": False, "delete": False}
        for m in ["dashboard", "projects", "tasks", "documents", "chat", "ai", "badges", "profile"]
    },
}
...
# Defaults for the newly-added module slugs so admins / managers / hr
# start with sensible access out-of-the-box even before they touch the matrix.
for _role in ("admin", "manager", "hr"):
    DEFAULT_MATRIX[_role]["outreach"] = {"view": True, "create": True, "edit": True, "delete": _role == "admin"}
    DEFAULT_MATRIX[_role]["newsletter"] = {"view": True, "create": True, "edit": True, "delete": _role == "admin"}
    # Iter 37 — Leave management: managers approve, admin manages policy
    DEFAULT_MATRIX[_role]["leave"] = {"view": True, "create": True, "edit": True, "delete": _role == "admin"}

# Everyone (including interns + team_members) can view + request leave for themselves
for _role in ("team_member", "intern"):
    DEFAULT_MATRIX[_role]["leave"] = {"view": True, "create": True, "edit": False, "delete": False}

# Iter 80 — Doc Studio is open to ALL portal users (per user's request)
for _role in ROLES:
    DEFAULT_MATRIX.setdefault(_role, {})
    DEFAULT_MATRIX[_role]["doc-studio"] = {"view": True, "create": True, "edit": True,
                                           "delete": _role in ("admin", "super_admin")}

# Interns can EDIT (status / start / progress) their assigned tasks even though
# they cannot create new ones. Override after the bulk init above.
DEFAULT_MATRIX["intern"]["tasks"] = {"view": True, "create": False, "edit": True, "delete": False}
# Read-only on projects (they only see what they're assigned to)
DEFAULT_MATRIX["intern"]["projects"] = {"view": True, "create": False, "edit": False, "delete": False}


async def ensure_intern_unified_uplift(db) -> None:
    """One-time idempotent migration: when the unified /app/* intern UX
    shipped (iter 34), interns gained tasks.edit + projects.view. If an
    operator already has a saved matrix from before this iteration, patch
    those two cells to True so existing intern accounts work immediately
    after deploy. Marker doc keeps this idempotent.
    """
    marker = await db.role_permissions.find_one({"id": "_migrations"}, {"_id": 0})
    flags = (marker or {}).get("flags", {})
    if flags.get("intern_unified_uplift_v1"):
        return
    saved = await db.role_permissions.find_one({"id": "active"}, {"_id": 0})
    if saved and isinstance(saved.get("matrix"), dict):
        matrix = saved["matrix"]
        intern = matrix.setdefault("intern", {})
        intern["tasks"] = {"view": True, "create": False, "edit": True, "delete": False}
        intern["projects"] = {"view": True, "create": False, "edit": False, "delete": False}
        await db.role_permissions.update_one(
            {"id": "active"},
            {"$set": {"matrix": matrix, "updated_at": datetime.now(timezone.utc).isoformat()}},
        )
    flags["intern_unified_uplift_v1"] = True
    await db.role_permissions.update_one(
        {"id": "_migrations"},
        {"$set": {"id": "_migrations", "flags": flags}},
        upsert=True,
    )
...
def register_rbac(api: APIRouter, db, get_current_user):
    ...
    async def _require_super_or_hr(user):
        if user.get("role") not in ("super_admin", "hr"):
            raise HTTPException(403, "Super admin or HR only")
    ...
    async def update_matrix(payload: MatrixIn, user=Depends(get_current_user)):
        ...
        # Bump a monotonic version so connected clients can detect a change
        # and bust their /rbac/permissions cache automatically.
        ...
        return {"ok": True, "matrix": clean, "version": version}

    @api.put("/rbac/matrix/role/{role}")
    async def update_matrix_role(role: str, payload: RoleMatrixIn, user=Depends(get_current_user)):
        """Role-scoped update — only the given role's permissions are written.
        Other roles keep their CURRENT server-side values (prevents stale
        full-matrix overwrites from another device bleeding across roles)."""
        await _require_super(user)
        if role not in ROLES:
            raise HTTPException(400, f"Unknown role: {role}")
        if role == "super_admin":
            raise HTTPException(400, "Super admin permissions are locked to full access")
        current = await get_role_matrix(db)
        clean_role: Dict[str, Dict[str, bool]] = {}
        for m in MODULES:
            cur = current.get(role, {}).get(m, {a: False for a in ACTIONS})
            sent = (payload.modules or {}).get(m, {})
            clean_role[m] = {a: bool(sent.get(a, cur.get(a, False))) for a in ACTIONS}
        current[role] = clean_role
        current["super_admin"] = {m: {a: True for a in ACTIONS} for m in MODULES}
        version = int(datetime.now(timezone.utc).timestamp() * 1000)
        await db.role_permissions.update_one(
            {"id": "active"},
            {"$set": {
        ...
        return {"ok": True, "role": role, "modules": clean_role, "version": version}
    ...
    async def my_permissions(user=Depends(get_current_user)):
        ...
        # Surface matrix version so connected clients can cheap-poll for changes
        meta = await db.role_permissions.find_one({"id": "active"}, {"_id": 0, "version": 1})
        return {
            "role": role,
    ...
    # Lightweight endpoint that ONLY returns the version — used by clients
    # to poll for matrix updates without re-downloading the full payload.
    @api.get("/rbac/version")
    async def matrix_version(user=Depends(get_current_user)):
        _ = user
        meta = await db.role_permissions.find_one({"id": "active"}, {"_id": 0, "version": 1})
        return {"version": (meta or {}).get("version", 0)}

    # ===== Public account-request (no auth) =====
    @api.post("/auth/request-account", status_code=201)
    async def request_account(payload: dict):
        """Public endpoint for prospective users to request an account.

        Stores the request in `account_requests` and pings every super_admin
        with an in-app notification so they can vet & provision the user from
        HR → Team Access.
        """
        ...
        if len(reason) < 10:
            raise HTTPException(400, "Please share a brief reason (min 10 chars)")
        # Rate limit: max 3 pending requests from the same email
        pending = await db.account_requests.count_documents({"email": email, "status": "pending"})
        if pending >= 3:
            raise HTTPException(429, "You already have a pending request — please wait for admin review")
        doc = {
            "id": str(uuid.uuid4()),
            "name": name[:120],
            "email": email[:200],
        ...
        await db.account_requests.insert_one(doc)
        # Notify every super_admin in-app
        try:
            admins = await db.users.find({"role": "super_admin"}, {"_id": 0, "id": 1}).to_list(20)
            for a in admins:
                await db.notifications.insert_one({
                    "id": str(uuid.uuid4()),
                    "owner_id": a["id"],
                    "kind": "account_request",
                    "title": f"New account request: {name}",
                    "message": f"{email}{' · ' + mobile if mobile else ''} — {reason[:160]}",
                    "link": "/app/hr?tab=team-access",
                    "read": False,
                    "created_at": datetime.now(timezone.utc).isoformat(),
                })
        except Exception:
            pass
        # Best-effort email to super_admin via Gmail (if connected)
        try:
            ...
            if token:
                ...
                body_html = f"""
<div style="font-family:Inter,Arial,sans-serif;color:#0F2042;max-width:580px;margin:0 auto;padding:24px;background:#FFFFFF">
  <div style="font-size:11px;font-weight:bold;letter-spacing:0.28em;color:#F97316;text-transform:uppercase">// PROJEXINO · ACCOUNT REQUEST</div>
  <h2 style="margin:8px 0 16px">{name} would like an account</h2>
  <table style="width:100%;border-collapse:collapse;font-size:14px">
    <tr><td style="padding:6px 0;color:#475569"><b>Email</b></td><td><a href="mailto:{email}">{email}</a></td></tr>
    <tr><td style="padding:6px 0;color:#475569"><b>Mobile</b></td><td>{mobile or '—'}</td></tr>
    <tr><td style="padding:6px 0;color:#475569;vertical-align:top"><b>Reason</b></td><td>{reason}</td></tr>
  </table>
  <p style="margin-top:20px"><a href="https://projexino.com/app/hr?tab=team-access" style="display:inline-block;background:#F97316;color:white;padding:10px 18px;border-radius:999px;text-decoration:none;font-weight:bold">Review in portal →</a></p>
</div>"""
                m = MIMEText(body_html, "html", "utf-8")
                m["To"] = token.get("email") or "admin@projexino.com"
                m["Subject"] = f"Account request: {name} ({email})"
    ...
    @api.get("/rbac/account-requests")
    async def list_account_requests(user=Depends(get_current_user), status: Optional[str] = "pending"):
        await _require_super(user)
        q: Dict = {}
        if status:
            q["status"] = status
        cur = db.account_requests.find(q, {"_id": 0}).sort("created_at", -1).limit(200)
        return await cur.to_list(200)

    @api.patch("/rbac/account-requests/{rid}")
    async def update_account_request(rid: str, payload: dict, user=Depends(get_current_user)):
        await _require_super(user)
        status = (payload.get("status") or "").strip()
        if status not in {"pending", "approved", "rejected"}:
            raise HTTPException(400, "Invalid status")
        r = await db.account_requests.update_one(
            {"id": rid},
            {"$set": {"status": status, "reviewed_by": user.get("email"), "reviewed_at": datetime.now(timezone.utc).isoformat()}},
        )
    ...
    async def list_admins(user=Depends(get_current_user)):
        """Team roster — visible to Super Admin and HR (Team Access tab)."""
        await _require_super_or_hr(user)
        cur = db.users.find(
    ...
    async def create_admin(payload: AdminCreateIn, user=Depends(get_current_user)):
        ...
        if user.get("role") == "hr" and (payload.role == "super_admin" or payload.is_primary):
            raise HTTPException(403, "HR cannot create super admin accounts")
    ...
    async def update_admin(admin_id: str, payload: AdminUpdateIn, user=Depends(get_current_user)):
        ...
        if user.get("role") == "hr":
            if target.get("role") == "super_admin":
                raise HTTPException(403, "HR cannot modify super admin accounts")
            if updates.get("role") == "super_admin" or updates.get("is_primary"):
                raise HTTPException(403, "HR cannot grant super admin")
    ...
    async def delete_admin(admin_id: str, user=Depends(get_current_user)):
        ...
        if not target:
            raise HTTPException(404, "Admin not found")
        if user.get("role") == "hr" and target.get("role") == "super_admin":
            raise HTTPException(403, "HR cannot remove super admin accounts")
