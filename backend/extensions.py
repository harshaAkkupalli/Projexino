...
class ProjectCreate(BaseModel):
    ...
    client: Optional[str] = ""
    client_id: Optional[str] = ""            # NEW · FK to clients collection
...
def register_extensions(api: APIRouter, db, get_current_user):
    ...
    async def update_project(project_id: str, payload: ProjectUpdate, user=Depends(get_current_user)):
        # owner / explicit manager / admin / manager-role can edit ANY field.
        # Assigned member or intern can edit status + description + start_date + deadline + tags
        # (per user request: members can update what's needed from the card view).
        ...
        is_priv = (
            ...
            or existing.get("manager_user_id") == uid
            or role in ("super_admin", "admin", "manager")
        )
        is_assigned = (
            uid in (existing.get("member_user_ids") or [])
            or uid in (existing.get("intern_user_ids") or [])
        )
        if not is_priv and not is_assigned:
            raise HTTPException(403, "Not allowed to edit this project")
        raw_updates = {k: v for k, v in payload.model_dump(exclude_unset=True).items() if v is not None}
        if not is_priv:
            # Assigned-only users can edit a limited subset.
            ASSIGNEE_EDITABLE = {"status", "description", "start_date", "deadline", "tags"}
            updates = {k: v for k, v in raw_updates.items() if k in ASSIGNEE_EDITABLE}
            if not updates:
                raise HTTPException(403, "You can only update status, description, dates or tags on this project")
        else:
            updates = raw_updates
        ...
        if "status" in updates and updates["status"] != existing.get("status"):
            try:
                ...
                pass
            # Also notify the manager + all members + interns (excluding actor)
            try:
                from notif_engine import notify
                stakeholder_ids = set()
                if existing.get("manager_user_id"):
                    stakeholder_ids.add(existing["manager_user_id"])
                stakeholder_ids.update(existing.get("member_user_ids") or [])
                stakeholder_ids.update(existing.get("intern_user_ids") or [])
                if existing.get("owner_id"):
                    stakeholder_ids.add(existing["owner_id"])
                stakeholder_ids.discard(user["id"])
                actor_name = user.get("name") or user.get("email") or "A teammate"
                for sid in stakeholder_ids:
                    sdoc = await db.users.find_one(
                        {"id": sid}, {"_id": 0, "id": 1, "email": 1, "name": 1}
                    )
                    if not sdoc:
                        continue
                    await notify(
                        ...
                        user_id=sdoc["id"],
                        user_email=sdoc.get("email", ""),
                        title=f"{existing['name']}: status {updates['status']}",
                        message=f"{actor_name} updated the project status from '{existing.get('status','')}' to '{updates['status']}'.",
                ...
                logger.exception("project_status_change notify failed")
    ...
    async def update_pipeline_stage(
        ...
        # Notify project manager + owner when a stage status changes (exclude actor)
        if "status" in updates_dict:
            try:
                from notif_engine import notify
                actor_name = user.get("name") or user.get("email") or "A team member"
                stakeholder_ids = set()
                if project.get("manager_user_id"):
                    stakeholder_ids.add(project["manager_user_id"])
                ...
                for sid in stakeholder_ids:
                    ...
                    await notify(
                        db,
                        event="project_stage_update",
                        user_id=sdoc["id"],
                        user_email=sdoc.get("email", ""),
                        title=f"{project['name']}: {stage['label']} → {updates_dict['status'].replace('_',' ')}",
                        message=f"{actor_name} updated the '{stage['label']}' stage status.",
                        link=f"/app/projects/{project_id}",
                        variables={
                            "name": sdoc.get("name", ""),
                            "project_name": project["name"],
                ...
                logger.exception("project_stage_update notify failed")
    ...
    @api.post("/documents/move")
    async def move_documents(body: dict, user=Depends(get_current_user)):
        """Organize selected documents into a named folder ('' = unfiled)."""
        ids = [str(x) for x in (body or {}).get("ids") or [] if str(x).strip()]
        folder = ((body or {}).get("folder") or "").strip()[:60]
        if not ids:
            raise HTTPException(400, "Select at least one document")
        r = await db.documents.update_many(
            {"owner_id": user["id"], "id": {"$in": ids}}, {"$set": {"folder": folder}})
        return {"ok": True, "moved": r.modified_count, "folder": folder}

    @api.post("/documents/share-email")
    async def share_documents_email(body: dict, user=Depends(get_current_user)):
        """Email documents via connected Gmail. folder → single {folder}.zip attachment;
        doc_ids → each file attached with its exact name."""
        ...
        to = [e.strip() for e in re.split(r"[,;\s]+", (body or {}).get("to") or "") if "@" in e]
        if not to:
            raise HTTPException(400, "Enter at least one recipient email")
        folder = ((body or {}).get("folder") or "").strip()
        doc_ids = [str(x) for x in (body or {}).get("doc_ids") or [] if str(x).strip()]
        subject = ((body or {}).get("subject") or "").strip() or (f"Documents · {folder}" if folder else "Documents from Projexino")
        message = ((body or {}).get("message") or "").strip()
        if folder:
            docs = await db.documents.find({"owner_id": user["id"], "folder": folder}, {"_id": 0}).to_list(200)
            if not docs:
                raise HTTPException(404, f"Folder '{folder}' has no documents")
            buf = io.BytesIO()
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                used = set()
                for d in docs:
                    n, base_n, k = d["name"], d["name"], 2
                    while n in used:
                        n = f"{k}-{base_n}"
                        k += 1
                    used.add(n)
                    zf.writestr(n, _b64.b64decode(d.get("content_base64") or ""))
            attachments = [{"filename": f"{folder}.zip", "content_type": "application/zip", "data": buf.getvalue()}]
        else:
            if not doc_ids:
                raise HTTPException(400, "Select files or a folder to share")
            docs = await db.documents.find({"owner_id": user["id"], "id": {"$in": doc_ids}}, {"_id": 0}).to_list(100)
            if not docs:
                raise HTTPException(404, "Selected documents not found")
            attachments = [{"filename": d["name"], "content_type": d.get("mime_type") or "application/octet-stream",
                            "data": _b64.b64decode(d.get("content_base64") or "")} for d in docs]
        if sum(len(a["data"]) for a in attachments) > 20 * 1024 * 1024:
            raise HTTPException(400, "Attachments exceed the 20 MB Gmail-safe limit — split the share")
        ...
        html = (f"<p>{message or 'Please find the attached document(s).'}</p>"
                f"<p style='color:#64748B;font-size:12px'>Sent from the Projexino portal by {user.get('name', '')}</p>")
        sent, errors = 0, []
        for rcpt in to:
            ...
            try:
                ...
                errors.append(f"{rcpt}: {str(e)[:120]}")
        if sent == 0:
            raise HTTPException(400, f"Sending failed — {'; '.join(errors)[:200]}")
        return {"ok": True, "sent": sent, "failed": errors, "attached": [a["filename"] for a in attachments]}
    ...
    # -------- Interns --------
    def _intern_scope(user) -> Dict[str, Any]:
        """Privileged roles operate on any intern; others only on those they own."""
        if user.get("role") in ("super_admin", "admin", "hr"):
            return {}
    ...
    async def list_intern_tasks(intern_id: str, user=Depends(get_current_user)):
        cur = db.intern_tasks.find(
            {"intern_id": intern_id, **_intern_scope(user)}, {"_id": 0}
        ).sort("created_at", -1)
        items = await cur.to_list(500)
    ...
    async def create_intern_task(payload: InternTaskCreate, user=Depends(get_current_user)):
        intern = await db.interns.find_one({"id": payload.intern_id, **_intern_scope(user)}, {"_id": 0})
        if not intern:
            raise HTTPException(404, "Intern not found")
        now = datetime.now(timezone.utc).isoformat()
        doc = payload.model_dump()
    ...
    async def update_intern_task(task_id: str, payload: InternTaskUpdate, user=Depends(get_current_user)):
        existing = await db.intern_tasks.find_one({"id": task_id, **_intern_scope(user)}, {"_id": 0})
        if not existing:
    ...
    async def intern_certificate(intern_id: str, user=Depends(get_current_user)):
        intern = await db.interns.find_one({"id": intern_id, **_intern_scope(user)}, {"_id": 0})
        if not intern:
            raise HTTPException(404, "Intern not found")
        tasks_cur = db.intern_tasks.find(
            {"intern_id": intern_id}, {"_id": 0}
        ).sort("created_at", 1)
...
def _fetch_logo_bytes():
    """Load Projexino logo PNG — embedded constant first (zero dependencies),
    then local files, then network; cached."""
    if _LOGO_CACHE["bytes"]:
        return _LOGO_CACHE["bytes"]
    try:
        from brand_assets import logo_bytes
        _LOGO_CACHE["bytes"] = logo_bytes()
        return _LOGO_CACHE["bytes"]
    except Exception:
        pass
    import os as _os
    for p in (_os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "assets", "projexino-logo.png"),
        ...
        try:
            with open(p, "rb") as f:
                _LOGO_CACHE["bytes"] = f.read()
            return _LOGO_CACHE["bytes"]
        except Exception:
            continue
    try:
        import urllib.request
        base = (_os.environ.get("PUBLIC_FRONTEND_URL")
                or _os.environ.get("REACT_APP_BACKEND_URL")
                or "").rstrip("/")
        url = f"{base}/projexino-logo.png"
        with urllib.request.urlopen(url, timeout=8) as r:
            _LOGO_CACHE["bytes"] = r.read()
    except Exception:
        return None
    return _LOGO_CACHE["bytes"] or None


def logo_data_uri(logo_url: str = "") -> str:
    """Resolve a letterhead logo to an embedded data: URI so WeasyPrint PDFs
    render with the logo even when the backend has no network route to the
    frontend (typical self-host / docker-compose setup)."""
    import base64 as _b
    if (logo_url or "").startswith("data:"):
        return logo_url
    if logo_url.startswith("http") and "emergentagent.com" not in logo_url:
        try:
            import urllib.request
            with urllib.request.urlopen(logo_url, timeout=8) as r:
                return "data:image/png;base64," + _b.b64encode(r.read()).decode("ascii")
        except Exception:
            pass
    b = _fetch_logo_bytes()
    if b:
        return "data:image/png;base64," + _b.b64encode(b).decode("ascii")
    return logo_url or ""
