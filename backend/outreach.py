...
def register_outreach(api: APIRouter, db, get_current_user):
    ...
    def _personalise(html: str, subject: str, lead: Dict[str, Any]) -> tuple[str, str]:
        """Replace personalisation variables (case-insensitive, with common aliases).
        Any unresolved {{placeholder}} is stripped so raw braces never reach an inbox."""
        first = (lead.get("first_name") or "").strip()
        last = (lead.get("last_name") or "").strip()
        full = (lead.get("name") or f"{first} {last}").strip() or "there"
        vars_ = {
            "firstname": first or "there", "first_name": first or "there",
            "lastname": last, "last_name": last,
            "name": full, "fullname": full, "full_name": full,
            "companyname": (lead.get("company") or "").strip() or "your team",
            "company": (lead.get("company") or "").strip() or "your team",
            "company_name": (lead.get("company") or "").strip() or "your team",
        ...
        def _sub(text: str) -> str:
            def repl(m):
                return vars_.get(m.group(1).strip().lower(), "")
            return re.sub(r"\{\{\s*([\w ]+?)\s*\}\}", repl, text or "")

        return _sub(subject), _sub(html)
    ...
    async def ai_write_email(payload: Dict[str, Any] = Body(...), user=Depends(_require_priv)):
        ...
        try:
            ...
            raw = await chat_completion(
                ...
                session_id=f"outreach-ai-{uuid.uuid4().hex[:8]}",
            )
            text = str(raw or "")
            # Extract JSON
    ...
    async def ai_reply_draft(lid: str, payload: Dict[str, Any] = Body(default={}), user=Depends(_require_priv)):
        ...
        try:
            ...
            raw = await chat_completion(
                ...
                session_id=f"outreach-ai-reply-{uuid.uuid4().hex[:8]}",
            )
            text = str(raw or "")
            json_block = re.search(r"\{[\s\S]*\}", text)
            if not json_block:
