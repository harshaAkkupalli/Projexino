...
import logging
logger = logging.getLogger("hr_letters")
...
def _fmt_letter_date(v: str | None) -> str:
    if not (v or "").strip():
        return ""
    try:
...
def _apply_letter_date(letter: Dict[str, Any]) -> str:
    """At PDF time, force the body's date line to match letter_date (if set)."""
    body = letter.get("body_html") or ""
    nd = _fmt_letter_date(letter.get("letter_date"))
    if not nd:
        return body
    new_p = f'<p class="pjx-letter-date" style="text-align:right;color:#64748B;font-size:9.5pt;margin:0 0 10px">{nd}</p>'
    tagged = _dre.compile(r'<p class="pjx-letter-date"[^>]*>.*?</p>', _dre.S)
    if tagged.search(body):
        return tagged.sub(new_p, body, count=1)
    legacy = _dre.compile(r'<p style="text-align:right;color:#64748B;font-size:9\.5pt;margin:0 0 10px">.*?</p>', _dre.S)
    if legacy.search(body):
        return legacy.sub(new_p, body, count=1)
    return new_p + body
...
def _design_letter_html(letter: Dict[str, Any], text: str) -> str:
    ...
    parts: List[str] = []
    today = _fmt_letter_date(letter.get("letter_date")) or datetime.now(timezone.utc).strftime("%d %B %Y")
    parts.append(f'<p class="pjx-letter-date" style="text-align:right;color:#64748B;font-size:9.5pt;margin:0 0 10px">{today}</p>')
    parts.append(
...
class LetterPatch(BaseModel):
    ...
    letter_date: Optional[str] = None  # YYYY-MM-DD shown on the letter (supports future dating)
...
def register_hr_letters(api: APIRouter, db, get_current_user):
    ...
    def _brand_html(letter: Dict[str, Any], profile: Dict[str, Any], hide_placed: bool = False) -> str:
        ...
        Signature blocks with x/y coordinates (percent of the A4 page, set via the
        drag-drop placement UI) are absolutely positioned on page 1; the rest flow
        into the classic bottom signature grid."""
        blocks = letter.get("signature_blocks") or []
        placed = [b for b in blocks if b.get("x") is not None and b.get("y") is not None]
        flowing = [b for b in blocks if b not in placed]
        blocks_html = ""
        for b in flowing:
        ...
        if not hide_placed:
            for b in placed:
                try:
                    x_pct = float(b.get("x")); y_pct = float(b.get("y"))
                except (TypeError, ValueError):
                    continue
                # % of full A4 page (210x297mm) → mm inside the content box (margins 20/22mm)
                left_mm = max(0.0, min(x_pct / 100.0 * 210 - 20, 168))
                top_mm = max(0.0, min(y_pct / 100.0 * 297 - 22, 245))
                sig = b.get("signature_data_url") or ""
                placed_html += f"""
                <div class="sigplaced" style="left:{left_mm:.1f}mm; top:{top_mm:.1f}mm;">
                  <div class="sigline">{('<img src="' + sig + '" />') if sig else '&nbsp;'}</div>
                  <div class="signame">{b.get('name', '')}</div>
                  <div class="sigrole">{b.get('role', b.get('label', ''))}</div>
                </div>"""

        logo_raw = profile.get("logo_url") or LOGO_URL
        from extensions import logo_data_uri
        logo = logo_data_uri(logo_raw)
...
.sigplaced {{ position: absolute; width: 42mm; }}
.sigplaced .sigline {{ height: 16mm; }}
...
<div class="body">{_apply_letter_date(letter)}</div>
...
{f'<div class="reg">{reg}</div>' if reg else ''}
    ...
    async def ai_draft(lid: str, payload: Dict[str, Any] = Body(default={}), user=Depends(_require_priv)):
        ...
        try:
            ...
            if not ai_configured():
                raise HTTPException(400, "No AI provider configured — add a key in Settings → AI")
            ...
            raw = await chat_completion(system_message=sys_msg,
                                        user_message=f"Draft the {TEMPLATES.get(template,{}).get('label','letter')} based on:\n\n{ctx}",
                                        session_id=f"hr-letter-{lid}")
    ...
    # ---------- PDF ----------
    def _stamp_letter_blocks(base_pdf: bytes, blocks: List[Dict[str, Any]]) -> bytes:
        """Stamp placed signature blocks (x/y % of page + 1-based page no.) onto their pages."""
        import io as _io, base64 as _b
        from pypdf import PdfReader, PdfWriter
        from reportlab.pdfgen import canvas as rl_canvas
        from reportlab.lib.utils import ImageReader
        reader = PdfReader(_io.BytesIO(base_pdf))
        n = len(reader.pages)
        ...
        for b in blocks:
            try:
                ...
                pg = 1
            by_page.setdefault(max(1, min(pg, n)), []).append(b)
        writer = PdfWriter()
        for i, page in enumerate(reader.pages, start=1):
            ...
            if todo:
                ...
                for b in todo:
                    try:
                        left = float(b["x"]) / 100.0 * pw; top = float(b["y"]) / 100.0 * ph
                    except (TypeError, ValueError, KeyError):
                        continue
                    box_w = pw * 0.20
                    img_h = 0.0
                    durl = b.get("signature_data_url") or ""
                    if durl.startswith("data:"):
                        try:
                            img = ImageReader(_io.BytesIO(_b.b64decode(durl.split(",", 1)[1])))
                            iw, ih = img.getSize()
                    ...
                    c.drawString(left, base_y - 12, (b.get("name") or b.get("label") or "")[:44])
                    c.setFont("Helvetica", 6.5); c.setFillColorRGB(0.42, 0.47, 0.55)
                    c.drawString(left, base_y - 21, (b.get("role") or b.get("label") or "")[:50])
                c.save(); buf.seek(0)
    ...
    async def _render_letter_pdf(d: Dict[str, Any], hide_placed: bool = False) -> bytes:
        profile = await _load_profile()
        from weasyprint import HTML
        base_url = (os.environ.get("PUBLIC_FRONTEND_URL")
                    or os.environ.get("REACT_APP_BACKEND_URL")
                    or "").rstrip("/")
        # Always render the base without placed blocks, then stamp them page-aware
        # (WeasyPrint absolute positioning only works on page 1; stamping supports any page).
        base = HTML(string=_brand_html(d, profile, hide_placed=True), base_url=base_url).write_pdf()
        if hide_placed:
            return base
        placed = [b for b in (d.get("signature_blocks") or []) if b.get("x") is not None and b.get("y") is not None]
        return _stamp_letter_blocks(base, placed) if placed else base

    @api.get("/hr/letters/{lid}/pdf")
    async def render_pdf(lid: str, inline: bool = False, hide_placed: bool = False, user=Depends(_require_priv)):
        d = await db.hr_letters.find_one({"id": lid, "is_deleted": {"$ne": True}}, {"_id": 0})
        if not d: raise HTTPException(404, "Not found")
        try:
            pdf = await _render_letter_pdf(d, hide_placed=hide_placed)
        except Exception as e:
    ...
    async def create_sign_token(lid: str, payload: Dict[str, Any] = Body(default={}), user=Depends(_require_priv)):
        ...
        shareable = bool((payload or {}).get("shareable"))
        ttl_min = 14 * 24 * 60 if shareable else SIGN_TOKEN_TTL_MIN
        token = secrets.token_urlsafe(24)
        await db.hr_sign_tokens.insert_one({
            "token": token, "letter_id": lid, "block_id": block_id, "signer_name": signer_name,
            "shareable": shareable,
            "created_by": {"id": user.get("id", ""), "email": user.get("email", ""), "name": user.get("name", "")},
        ...
        return {"token": token, "url": f"{base}/sign/{token}", "expires_in_min": ttl_min}
    ...
    async def get_by_token(token: str):
        ...
        return {"letter_title": (d or {}).get("title", ""), "signer_name": t.get("signer_name", ""),
                "block_id": t.get("block_id", ""), "shareable": bool(t.get("shareable")), "has_pdf": True}

    @api.get("/public/hr-letters/sign/{token}/pdf")
    async def public_sign_pdf(token: str):
        t = await db.hr_sign_tokens.find_one({"token": token}, {"_id": 0})
        if not t: raise HTTPException(404, "Invalid link")
        if t.get("used"): raise HTTPException(410, "This link has already been used")
        try:
        ...
        d = await db.hr_letters.find_one({"id": t["letter_id"], "is_deleted": {"$ne": True}}, {"_id": 0})
        if not d: raise HTTPException(404, "Letter not found")
        try:
            pdf = await _render_letter_pdf(d)
        except Exception as e:
            raise HTTPException(500, f"PDF render failed: {str(e)[:200]}")
        return Response(content=pdf, media_type="application/pdf",
    ...
    async def public_sign(token: str, payload: Dict[str, Any] = Body(...)):
        ...
        signer = (payload or {}).get("signer_name") or t.get("signer_name") or "External signer"
        await db.hr_letters.update_one({"id": d["id"]}, {"$set": {
            "signature_blocks": blocks, "updated_at": _now_iso(),
            "sign_status": "signed",
            "last_signed": {"name": signer, "at": _now_iso(), "via": "public_link"}}})
        await db.hr_sign_tokens.update_one({"token": token}, {"$set": {"used": True, "used_at": _now_iso()}})
        # Portal notification for the person who created the link
        creator = t.get("created_by") or {}
        if creator.get("id"):
            await db.notifications.insert_one({
                "id": uuid.uuid4().hex, "owner_id": creator["id"], "kind": "letter_signed",
                "title": "Document signed ✍️",
                "message": f'{signer} signed "{d.get("title", "your letter")}"',
                "link": "/app/hr-letters", "read": False, "created_at": _now_iso(),
            })
        # Best-effort email notification (needs a connected Gmail account)
        email_notified = False
        notify_to = t.get("notify_email") or creator.get("email")
        if notify_to:
            try:
                import base64 as _b64
                from email.mime.text import MIMEText
                from email_module import _resolve_send_token, _refresh_if_needed, _build_gmail_service
                tok = await _resolve_send_token(db, None)
                if tok:
                    tok = await _refresh_if_needed(db, tok)
                    service = _build_gmail_service(tok)
                    m = MIMEText(
                        f'<p><b>{signer}</b> has signed the document "<b>{d.get("title", "")}</b>" via your shared signing link.</p>'
                        f'<p>The signature is already placed on the letter — open HR → Letters in the portal to view or download the signed PDF.</p>',
                        "html")
                    m["Subject"] = f'✍️ Signed: {d.get("title", "Document")} — by {signer}'
                    m["From"] = f'"Projexino Sign" <{tok.get("email")}>'
                    m["To"] = notify_to
                    service.users().messages().send(userId="me", body={
                        "raw": _b64.urlsafe_b64encode(m.as_bytes()).decode("ascii")}).execute()
                    email_notified = True
            except Exception:
                logger.warning("Sign notification email failed (Gmail not connected?)")
        return {"ok": True, "email_notified": email_notified}
