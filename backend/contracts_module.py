...
<div class="brand">
  <div>
    <img class="logo-img" src="{__import__('extensions').logo_data_uri(profile['logo_url'])}" alt="{profile.get('company_name','')}"/>
...
{f'<div class="reg">{reg_html}</div>' if reg_html else ''}
    ...
    async def ai_draft_email(cid: str, payload: ContractAiEmailPayload, user=Depends(_require_priv)):
        ...
        try:
            ...
            raw = await chat_completion(system_message=sys_msg, user_message=f"Draft the email now.\n\n{ctx}",
                                        session_id=f"contract-email-{cid}")
