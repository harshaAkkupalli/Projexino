...
def _fetch_projexino_logo() -> Optional[bytes]:
    """Download the Projexino logo PNG once and cache the bytes in-process."""
    global _PROJEXINO_LOGO_CACHE
    if _PROJEXINO_LOGO_CACHE is not None:
        return _PROJEXINO_LOGO_CACHE or None
    try:
        import urllib.request
        with urllib.request.urlopen(PROJEXINO_LOGO_URL, timeout=5) as resp:
            _PROJEXINO_LOGO_CACHE = resp.read()
    except Exception:
        _PROJEXINO_LOGO_CACHE = b""  # mark as failed so we don't retry
    return _PROJEXINO_LOGO_CACHE or None
