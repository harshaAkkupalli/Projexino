...
def register_email(api: APIRouter, db, get_current_user):
    ...
    async def _do_send(*, to: list, subject: str, body_html: str, from_name: str, reply_to: str, user=None, from_token_id: str = None, cc: list = None, bcc: list = None, attachments: list = None):
        ...
        if attachments:
            ...
            for h in ("Subject", "From", "To", "Cc", "Reply-To"):
                if msg.get(h):
                    mixed[h] = msg[h]
            mixed.attach(related)
            for a in attachments:
                try:
                    main, _, sub = (a.get("content_type") or "application/octet-stream").partition("/")
                    ...
                    logger.warning("Could not attach %s", a.get("filename"))
            msg = mixed
        else:
