...
import { downloadApiPdf, downloadBrandedCsv, openApiPdfInTab } from "@/lib/download";
                ...
                <tr key={s.id} data-testid={`hr-slip-${s.slip_no}`} className="border-t border-slate-100 hover:bg-orange-50/40">
                  ...
                  <td className="px-4 py-2 text-right">
                    <button onClick={async () => {
                        try { await openApiPdfInTab(`/hr/payslips/${s.id}/pdf`); }
                        catch (e) { toast.error(formatApiError(e)); }
                      }}
                      data-testid={`hr-slip-preview-${s.slip_no}`}
                      className="mr-1.5 inline-flex items-center gap-1 rounded-full border border-slate-200 px-3 py-1 text-[11px] font-bold text-slate-600 hover:border-[#0F2042]">
...
function PayslipConfigModal({ cfg, onClose, onSaved }) {
  ...
  const [sigy, setSigy] = useState(cfg.signatory || { name: "", designation: "Authorised Signatory", signature_data_url: "" });
  const sigPadRef = useRef(null);
  const [busy, setBusy] = useState(false);
  const captureSig = () => {
    if (!sigPadRef.current || sigPadRef.current.isEmpty()) return toast.error("Draw a signature first");
    setSigy((s) => ({ ...s, signature_data_url: sigPadRef.current.getCanvas().toDataURL("image/png") }));
    toast.success("Signature captured — it will appear on every payslip");
  };
  const save = async () => {
    setBusy(true);
    try {
      ...
      await api.put("/hr/payslip-config/signatory", sigy);
      toast.success("Saved"); onSaved();
  ...
  return (
    <ModalShell title="Payslip configuration" onClose={onClose} max="max-w-3xl">
      <div className="space-y-4">
        ...
        <div className="rounded-xl bg-violet-50 p-3" data-testid="hr-signatory-section">
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-violet-700">Authorised signatory — signed once, applied to every payslip</div>
          <div className="mt-2 grid gap-3 md:grid-cols-2">
            <div>
              <div className="rounded-xl border border-slate-300 bg-white">
                <SignatureCanvas ref={sigPadRef} penColor="#0F2042" canvasProps={{ width: 300, height: 90, className: "w-full", "data-testid": "hr-signatory-pad" }} />
              </div>
              <div className="mt-1.5 flex items-center gap-1.5">
                <button type="button" onClick={captureSig} data-testid="hr-signatory-use"
                  className="rounded-full bg-violet-600 px-3 py-1 text-[11px] font-bold text-white">Use signature</button>
                <button type="button" onClick={() => { sigPadRef.current?.clear(); setSigy((s) => ({ ...s, signature_data_url: "" })); }}
                  data-testid="hr-signatory-clear"
                  className="rounded-full border border-slate-200 px-3 py-1 text-[11px] font-bold text-slate-600">Clear</button>
                {sigy.signature_data_url && (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-700" data-testid="hr-signatory-saved">
                    <CheckCircle2 size={11}/> On file
                    <img src={sigy.signature_data_url} alt="signature" className="ml-1 h-7 rounded border border-slate-200 bg-white px-1" />
                  </span>
                )}
              </div>
            </div>
            <div className="space-y-2">
              <TF label="Signatory name" v={sigy.name || ""} on={(v) => setSigy({ ...sigy, name: v })} testId="hr-signatory-name" />
              <TF label="Designation" v={sigy.designation || ""} on={(v) => setSigy({ ...sigy, designation: v })} testId="hr-signatory-desig" />
              <p className="text-[10px] text-slate-500">Employee signature is no longer printed on payslips — only this authorised signatory.</p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => setSigning(d)} data-testid={`hr-sign-go-${d.id}`}
                    className="rounded-full bg-[#0F2042] px-3 py-1 text-[11px] font-bold text-white">{d.i_have_signed ? "Preview" : "Preview & sign"}</button>
                </div>
              </div>
              {qrDoc === d.id && qrToken && (
      ...
      <AnimatePresence>
        ...
        {signing && <SignDocPreviewModal doc={signing} onClose={() => setSigning(null)} onChanged={load} />}
      </AnimatePresence>
    </div>
  );
}
...
function SignDocPreviewModal({ doc, onClose, onChanged }) {
  const { user } = useAuth();
  const [d, setD] = useState(doc);
  const [refreshKey, setRefreshKey] = useState(0);
  const canManage = ["super_admin", "admin", "hr"].includes(user?.role);
  const mySig = (d.signatures || []).find((s) => s.user_id === user?.id);
  const reload = async () => {
    try {
      const { data } = await api.get("/hr/sign-docs");
      const nd = (data || []).find((x) => x.id === doc.id);
      if (nd) setD(nd);
      setRefreshKey((k) => k + 1);
      onChanged?.();
  ...
  return (
    <ModalShell title={`${mySig ? "Preview" : "Sign"}: ${d.name}`} onClose={onClose} max="max-w-3xl">
      <DocSignStudio
        fetchPdf={async () => (await api.get(`/hr/sign-docs/${d.id}/pdf?hide_placed=true`, { responseType: "arraybuffer" })).data}
        signatures={d.signatures || []}
        mySigId={mySig?.id || null}
        signed={!!mySig}
        canManage={canManage}
        defaultName={user?.name || ""}
        refreshKey={refreshKey}
        onSign={async (p) => { await api.post(`/hr/sign-docs/${d.id}/sign`, p); toast.success("Signed"); await reload(); }}
        onPlace={async (positions) => {
          const { data } = await api.patch(`/hr/sign-docs/${d.id}/signatures`, { positions });
          setD((x) => ({ ...x, signatures: data.signatures }));
          toast.success("Positions saved — check the PDF");
          onChanged?.();
        }}
      />
    </ModalShell>
...
function AuditTab() {
  ...
  const load = async () => {
    try {
      const [a, r] = await Promise.all([api.get(`/hr/audit/${year}`), api.get(`/hr/audit-records?year=${year}`)]);
      setData(a.data); setRecords(r.data);
    } catch { toast.error("Failed"); }
  };
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [year]);
  const removeRecord = async (rid) => {
    try {
      await api.delete(`/hr/audit-records/${rid}`);
      toast.success("Removed from audit list");
      load();
    } catch (e) { toast.error(formatApiError(e)); }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button data-testid="hr-audit-csv" onClick={() => downloadBrandedCsv({
            ...
            rows: [
              ...
              ["", ""],
              ["Month", "Expenses (INR)", "Payslip outflow (INR)", "Receipts income (INR)"],
              ...data.monthly.map((m) => [m.month, m.expense || 0, m.payslip_total || 0, m.receipt_income || 0]),
              ["", ""],
              ["Audit Records (receipts)", "", "", ""],
              ["Receipt No", "Client", "Amount", "Date"],
              ...records.map((r) => [r.receipt_no, r.client_name || "", `${r.currency} ${r.amount}`, r.record_date]),
            ],
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500">
            <tr><th className="px-3 py-2 text-left">Month</th><th className="px-3 py-2 text-right">Expenses</th><th className="px-3 py-2 text-right">Payslip outflow</th><th className="px-3 py-2 text-right">Receipts income</th></tr>
          </thead>
          <tbody>
            {data.monthly.map((m) => (
              <tr key={m.month} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-right font-semibold text-emerald-700">₹{(m.receipt_income || 0).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="rounded-2xl border border-violet-200 bg-white" data-testid="audit-records-section">
        <div className="flex items-center justify-between border-b border-violet-100 bg-violet-50/60 px-4 py-2.5 rounded-t-2xl">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-violet-700">// audit records — receipts</div>
            <p className="text-[11px] text-slate-500">Payment receipts pushed here from Finance → Receipts ("Add to Audit").</p>
          </div>
          <span className="rounded-full bg-violet-100 px-2.5 py-0.5 text-[10px] font-bold text-violet-700">{records.length} record{records.length !== 1 ? "s" : ""}</span>
        </div>
        {records.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-400" data-testid="audit-records-empty">
            No receipts recorded yet — open Finance → Receipts and click "Add to Audit" on any receipt.
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {records.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center gap-3 px-4 py-2.5" data-testid={`audit-record-${r.id}`}>
                <Receipt size={15} className="shrink-0 text-violet-500" />
                <div className="min-w-[150px] flex-1">
                  <div className="text-sm font-bold text-[#0F2042]">{r.receipt_no} <span className="ml-1 text-[10px] font-normal text-slate-400">Ref {r.invoice_no || "—"}</span></div>
                  <div className="text-[11px] text-slate-500">{r.client_name || "—"}{r.project_name ? ` · ${r.project_name}` : ""} · {r.record_date} · added by {r.added_by?.name || "—"}</div>
                </div>
                <span className="shrink-0 font-bold text-emerald-600">{r.currency === "INR" ? "₹" : `${r.currency} `}{Number(r.amount || 0).toLocaleString()}</span>
                <button data-testid={`audit-record-del-${r.id}`} onClick={() => removeRecord(r.id)} title="Remove from audit list"
                  className="rounded-lg border border-rose-200 p-1.5 text-rose-400 hover:bg-rose-50"><Trash2 size={12} /></button>
              </div>
            ))}
          </div>
        )}
