      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 md:px-8">
          <div className="flex items-center gap-3">
            <button className="md:hidden" onClick={() => setOpen(true)} data-testid="portal-menu-toggle">
              <Menu size={20} />
            </button>
            <GlobalSearch />
          </div>
  ...
  return (
    <div className="relative hidden md:block">
      ...
      <input
        ...
        className="w-80 rounded-lg border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-[#F97316] focus:ring-1 focus:ring-[#F97316]"
      />
      {open && q.trim() && (
        <div data-testid="global-search-results" className="absolute left-0 top-12 z-50 max-h-[70vh] w-[480px] overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-2xl">
          {loading ? (
