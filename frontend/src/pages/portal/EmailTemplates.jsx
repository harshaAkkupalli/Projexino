            ...
            >
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                {t.attachment_count > 0 && (
                  <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-orange-50 px-2 py-0.5 text-[9px] font-bold text-orange-700 ring-1 ring-orange-200" data-testid={`tpl-attach-count-${t.slug || t.id}`}>
                    <Paperclip size={8} /> {t.attachment_count} attachment{t.attachment_count > 1 ? "s" : ""}
                  </div>
                )}
...
function PreviewModal({ template, onClose, onSendTest }) {
  const [width, setWidth] = useState(typeof window !== "undefined" && window.innerWidth < 600 ? "phone" : "desktop");
  const widthPx = { desktop: 720, tablet: 540, phone: 380 }[width];
  const scrollRef = useRef(null);
  const [cw, setCw] = useState(0);
  useEffect(() => {
    const measure = () => setCw(scrollRef.current?.clientWidth || 0);
    measure();
    window.addEventListener("resize", measure);
    return () => window.removeEventListener("resize", measure);
  }, []);
  const scale = cw ? Math.min(1, (cw - 24) / widthPx) : 1;
  const frameH = 1100;
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        {/* SCROLL CONTAINER — scales the email to always fit the viewport width */}
        <div ref={scrollRef} className="flex-1 overflow-auto bg-gradient-to-br from-slate-100 to-slate-50">
          <div className="flex min-h-full justify-center px-3 py-6">
            <div style={{ width: `${widthPx * scale}px`, height: `${frameH * scale + 12}px` }}>
              <div
                style={{ width: `${widthPx}px`, transform: `scale(${scale})`, transformOrigin: "top left" }}
                className="rounded-2xl bg-white shadow-xl ring-1 ring-slate-200"
                data-testid="tpl-preview-canvas"
              >
                <iframe
                  title="Email preview"
                  srcDoc={renderPreview(template.body_html)}
                  style={{ width: "100%", height: `${frameH}px`, display: "block", border: 0, borderRadius: "16px" }}
                />
              </div>
            </div>
          </div>
...
function EditorModal({ template, onClose, onSaved, onSendTest }) {
  ...
  const uploadPending = async (tid) => {
    for (const f of pendingFiles) {
      try { await uploadTemplateAttachment(tid, f); }
      catch (err) { toast.error(err?.response?.data?.detail || err.message || `Failed to attach ${f.name}`); }
    }
    setPendingFiles([]);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (pendingFiles.length) await uploadPending(data.id);
      ...
      >
        ...
        <div className="grid max-h-[78vh] grid-cols-1 gap-0 overflow-y-auto lg:grid-cols-[1.05fr_1fr]">
          ...
          <div className="space-y-4 p-6">
            ...
            <AttachmentsSection templateId={editing ? template.id : null} pending={pendingFiles} setPending={setPendingFiles} />
...
function SendTestModal({ template, onClose }) {
  ...
  useEffect(() => {
    api.get(`/email/templates/${template.id}/attachments`).then(({ data }) => setAttachList(data || [])).catch(() => {});
  }, [template.id]);
  ...
  const variables = useMemo(() => {
    const found = new Set(template.variables_hint || []);
    const rx = /\{\{\s*([\w ]+?)\s*\}\}/g;
    for (const src of [template.subject || "", template.body_html || ""]) {
      let m;
      while ((m = rx.exec(src))) found.add(m[1].trim());
    }
    return [...found];
  }, [template]);
  ...
  return (
    ...
    >
      ...
      >        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#F97316]">// send email</div>
        ...
        <div className="space-y-4 p-6">
          ...
          {attachList.length > 0 && (
            <div className="rounded-xl border border-orange-200 bg-orange-50 p-3" data-testid="tpl-test-attachments">
              <div className="mb-1 text-[10px] font-bold uppercase tracking-wide text-orange-700">
                <Paperclip size={10} className="mr-1 inline" /> {attachList.length} file{attachList.length > 1 ? "s" : ""} will be attached
              </div>
              <div className="flex flex-wrap gap-1">
                {attachList.map((a) => (
                  <span key={a.id} className="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-orange-800 ring-1 ring-orange-200">{a.filename}</span>
                ))}
              </div>
            </div>
          )}
          <Field label="To (multiple — comma separated) *" value={to} onChange={setTo} testId="tpl-test-to" placeholder="a@company.com, b@company.com" />
          <Field label="CC (optional — comma separated)" value={cc} onChange={setCc} testId="tpl-test-cc" placeholder="manager@company.com" />
          {variables.length > 0 && (
            <div>
...
const ATTACH_ACCEPT = ".png,.jpg,.jpeg,.webp,.gif,.pdf,.zip,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt";

export async function uploadTemplateAttachment(templateId, file) {
  if (file.size > 10 * 1024 * 1024) throw new Error(`${file.name} is over the 10 MB per-file limit`);
  const data_b64 = await readFileAsB64(file);
  const { data } = await api.post(`/email/templates/${templateId}/attachments`, {
    filename: file.name, content_type: file.type || "", data_b64,
  });
  return data;
}

function AttachmentsSection({ templateId, pending, setPending }) {
  ...
  const reload = async () => {
    ...
    try {
      const { data } = await api.get(`/email/templates/${templateId}/attachments`);
      setItems(data);
    } catch { /* silent */ }
  };
  useEffect(() => { reload(); }, [templateId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const onPick = async (e) => {
    ...
    if (tooBig) { toast.error(`${tooBig.name} exceeds the 10 MB per-file limit`); return; }
    if (!templateId) {
      setPending((p) => [...p, ...files]);
      toast.success(`${files.length} file(s) queued — uploaded when you save the template`);
      return;
    }
    setBusy(true);
    for (const f of files) {
      try { await uploadTemplateAttachment(templateId, f); toast.success(`Attached ${f.name}`); }
      catch (err) { toast.error(formatApiError(err?.response?.data?.detail) || err.message || `Failed to attach ${f.name}`); }
    }
    setBusy(false);
    reload();
  };
  ...
  const remove = async (a) => {
    await api.delete(`/email/templates/${templateId}/attachments/${a.id}`);
    setItems((p) => p.filter((x) => x.id !== a.id));
    toast.success("Attachment removed");
  };

  const download = async (a) => {
    const res = await api.get(`/email/templates/${templateId}/attachments/${a.id}/download`, { responseType: "blob" });
    await saveOrShareBlob(new Blob([res.data], { type: a.content_type || "application/octet-stream" }), a.filename, a.content_type || "application/octet-stream");
  };

  return (
    <div data-testid="tpl-attachments-section">
      <div className="mb-1 flex items-center justify-between">
        <span className="text-xs uppercase tracking-[0.2em] text-slate-500">
          <Paperclip size={11} className="mr-1 inline" /> Attachments <span className="normal-case text-slate-400">(sent with every email using this template)</span>
        </span>
        <label className={`inline-flex cursor-pointer items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600 hover:bg-slate-200 ${busy ? "pointer-events-none opacity-60" : ""}`}>
          {busy ? <Loader2 size={10} className="animate-spin" /> : <Upload size={10} />} Add files
          <input type="file" multiple accept={ATTACH_ACCEPT} className="hidden" onChange={onPick} data-testid="tpl-attach-input" />
        </label>
      </div>
      {items.length === 0 && (pending || []).length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-200 px-3 py-2 text-[11px] text-slate-400">
          No files attached. Images, PDFs, docs or ZIPs — up to 10 MB each, 20 MB total.
        </div>
      ) : (
        <div className="flex flex-wrap gap-1.5">
          {items.map((a) => (
            <span key={a.id} data-testid={`tpl-attach-${a.id}`} className="inline-flex items-center gap-1.5 rounded-full bg-orange-50 px-2.5 py-1 text-[10px] font-bold text-orange-800 ring-1 ring-orange-200">
              <Paperclip size={9} /> {a.filename} <span className="font-normal text-orange-500">{fmtSize(a.size)}</span>
              <button onClick={() => download(a)} className="text-slate-500 hover:text-[#0F2042]" title="Download"><Download size={10} /></button>
              <button onClick={() => remove(a)} data-testid={`tpl-attach-del-${a.id}`} className="text-rose-400 hover:text-rose-600" title="Remove"><X size={10} /></button>
            </span>
          ))}
          {(pending || []).map((f, i) => (
            <span key={`p${i}`} className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600 ring-1 ring-slate-200">
              <Paperclip size={9} /> {f.name} <span className="font-normal text-slate-400">{fmtSize(f.size)} · queued</span>
              <button onClick={() => setPending((p) => p.filter((_, j) => j !== i))} className="text-rose-400 hover:text-rose-600"><X size={10} /></button>
            </span>
          ))}
        </div>
      )}
...
const LOGO_URL = `${window.location.origin}/projexino-logo.png`;
function substituteLogo(html = "") {
  return html.replace(/\{\{\s*logo_url\s*\}\}/g, LOGO_URL);
}

function renderPreview(html = "") {
  // If already contains the full shell, use as-is.
  if (/<html|<body/i.test(html)) return substituteLogo(html);
  return `<!doctype html>
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#FFF7ED;padding:24px 0;">
    <tr><td align="center">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;background:white;border-radius:16px;box-shadow:0 8px 24px rgba(15,32,66,0.08);overflow:hidden;">
        <tr><td style="background:#FFF7ED;border-top:6px solid #0F2042;border-bottom:3px solid #F97316;padding:22px 32px;">
          <img src="${LOGO_URL}" alt="Projexino" height="40" style="display:block;max-width:230px"/>
          <div style="font-size:10px;font-weight:700;letter-spacing:0.14em;color:#475569;margin-top:8px;">ENGINEERING STUDIOS · DEDICATED TEAMS · AI WORKFLOWS</div>
        </td></tr>
