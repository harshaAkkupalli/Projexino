...
import {
  ...
  Settings as SettingsIcon, Save, Upload, ExternalLink, Move, Link2,
} from "lucide-react";
import { api, formatApiError } from "@/lib/api";
import { downloadApiPdf, openApiPdfInTab } from "@/lib/download";
...
function LetterEditor({ id, profile, onClose, onSaved }) {
  ...
  const openPdf = async () => {
    try { await openApiPdfInTab(`/hr/letters/${id}/pdf?inline=true`); }
    catch (e) { toast.error(e.message || "Could not open PDF"); }
  };
  const shareSignLink = async () => {
    try {
      const { data } = await api.post(`/hr/letters/${id}/sign-token`, { shareable: true });
      await navigator.clipboard.writeText(data.url).catch(() => {});
      setQrToken({ url: data.url, expires: data.expires_in_min, blockId: "", shareable: true });
      toast.success("Sign link copied — valid 14 days. Anyone with it (even outside the org) can sign.");
    } catch (e) { toast.error(formatApiError(e)); }
  };
  const addSigBlock = () => {
  ...
  const addSavedSignatory = async () => {
    try {
      const { data } = await api.get("/hr/payslip-config");
      const s = data?.signatory || {};
      if (!s.signature_data_url) return toast.error("No saved signatory yet — draw one in HR → Payslips → Configure");
      const nb = {
        id: crypto.randomUUID(),
        label: s.designation || "Authorised Signatory",
        name: s.name || "Authorised Signatory",
        role: s.designation || "Authorised Signatory",
        signature_data_url: s.signature_data_url,
      };
      patch({ signature_blocks: [...(d.signature_blocks || []), nb] });
      toast.success(`${nb.name}'s signature added — drag it onto the document`);
    } catch (e) { toast.error(formatApiError(e)); }
  };
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={shareSignLink} data-testid="hrl-share-sign"
              className="inline-flex items-center gap-1 rounded-full border border-emerald-300 bg-white px-3 py-1.5 text-xs font-bold text-emerald-700 hover:bg-emerald-50">
              <Link2 size={11}/> Share sign link
            </button>
            <button onClick={openPdf} data-testid="hrl-open-tab"
              className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold hover:border-[#0F2042]">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {showDetails && (
            <div className="mt-3 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:grid-cols-2" data-testid="hrl-details-panel">
              ...
              <T label="Letter date (shown on PDF — future dates allowed)" v={d.letter_date || ""} o={(v) => patch({ letter_date: v })} t="hrl-edit-letterdate" type="date"/>
        ...
        {/* Inline document + drag-drop signature placement */}
        <InlinePlacement letterId={id} blocks={d.signature_blocks || []}
          refreshKey={`${d.updated_at}-${(d.signature_blocks || []).length}`}
          onSaved={(blocks) => patch({ signature_blocks: blocks })} />
      </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button onClick={addSavedSignatory} data-testid="hrl-sig-add-signatory"
                title="Insert the authorised signatory saved in Payslips config — already signed"
                className="inline-flex items-center gap-1 rounded-full bg-violet-600 px-2 py-1 text-[10px] font-bold text-white hover:bg-violet-700">
                <PenLine size={10}/> Saved signatory
              </button>
...
// ====================== SIGNATURE PLACEMENT (drag & drop, multi-page) ======================
function InlinePlacement({ letterId, blocks, refreshKey, onSaved }) {
  ...
  useEffect(() => {
    const p = {};
    (blocks || []).forEach((b) => { p[b.id] = (b.x != null && b.y != null) ? { x: b.x, y: b.y, page: b.page || 1 } : null; });
    setPos(p);
  }, [blocks]);
  const dragRef = useRef(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get(`/hr/letters/${letterId}/pdf?hide_placed=true`, { responseType: "arraybuffer" });
        const pdf = await pdfjsLib.getDocument({ data: res.data }).promise;
        if (cancelled) return;
        setPageCount(pdf.numPages);
        await new Promise((r) => setTimeout(r, 60)); // let page canvases mount
        const targetW = Math.min(560, window.innerWidth - 80);
      ...
      } catch { toast.error("Could not load the letter preview"); }
    })();
    return () => { cancelled = true; };
  }, [letterId, refreshKey, reload]);

  const pctFromEvent = (e, idx) => {
    const r = wrapRefs.current[idx].getBoundingClientRect();
    const cx = (e.touches ? e.touches[0].clientX : e.clientX) - r.left;
    const cy = (e.touches ? e.touches[0].clientY : e.clientY) - r.top;
    return { x: Math.max(0, Math.min(80, (cx / r.width) * 100)), y: Math.max(0, Math.min(92, (cy / r.height) * 100)), page: idx + 1 };
  };
  const startDrag = (bid, idx) => (e) => {
    e.preventDefault(); dragRef.current = bid;
    if (idx != null) setPos((p) => ({ ...p, [bid]: pctFromEvent(e, idx) }));
  };
  const onMove = (idx) => (e) => {
    if (!dragRef.current) return;
    const xy = pctFromEvent(e, idx);
  ...
  const save = async () => {
    ...
    try {
      const updated = blocks.map((b) => {
        const p = pos[b.id];
        return p
          ? { ...b, x: Math.round(p.x * 10) / 10, y: Math.round(p.y * 10) / 10, page: p.page || 1 }
          : { ...b, x: null, y: null, page: null };
      });
      await api.patch(`/hr/letters/${letterId}`, { signature_blocks: updated });
      toast.success("Signature positions saved — check the PDF");
      onSaved(updated);
    } catch (e) { toast.error(formatApiError(e)); }
    setSaving(false);
  };
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" data-testid="hrl-placement-inline">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-5 py-3.5">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#F97316]">// document · drag signatures onto any page</div>
          <p className="text-xs text-slate-500">Sign in the sidebar (pad or QR), then drag the chip anywhere on <b>any page</b> and hit <b>Save positions</b>.</p>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={() => setReload((r) => r + 1)} title="Refresh document preview" data-testid="hrl-placement-refresh"
            className="inline-flex items-center gap-1 rounded-full border border-slate-200 px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:border-[#0F2042]">
            <RefreshCw size={11}/> Refresh
          </button>
          <button onClick={save} disabled={saving} data-testid="hrl-placement-save"
            className="inline-flex items-center gap-1.5 rounded-full bg-[#0F2042] px-4 py-1.5 text-[11px] font-bold text-white disabled:opacity-50">
            {saving ? <Loader2 size={12} className="animate-spin"/> : <Save size={12}/>} Save positions
          </button>
        </div>
      </div>
      {/* tray of unplaced chips */}
      <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-100 bg-slate-50/60 px-5 py-2" data-testid="hrl-placement-tray">
        {blocks.filter((b) => !pos[b.id]).map((b) => (
          <button key={b.id} data-testid={`hrl-chip-${b.id}`} onMouseDown={startDrag(b.id, null)} onTouchStart={startDrag(b.id, null)}
            className="cursor-grab rounded-full bg-violet-600 px-3 py-1 text-[10px] font-bold text-white shadow active:cursor-grabbing">
            ✍ {b.label || b.name || "Signer"}
          </button>
        ))}
        {blocks.length === 0 && <span className="text-[10px] text-slate-400">Add a signature block in the sidebar first.</span>}
        {blocks.length > 0 && blocks.every((b) => pos[b.id]) && <span className="text-[10px] text-slate-400">All signatures placed — drag to reposition, even across pages.</span>}
      </div>
      <div className="space-y-4 overflow-x-auto p-4" onMouseUp={endDrag} onMouseLeave={endDrag} onTouchEnd={endDrag}>
        {Array.from({ length: Math.max(pageCount, 1) }, (_, i) => (
          <div key={i}>
            {pageCount > 1 && <div className="mb-1 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400">Page {i + 1} of {pageCount}</div>}
            <div ref={(el) => { wrapRefs.current[i] = el; }} data-testid={`hrl-page-${i + 1}`}
              className="relative mx-auto w-fit select-none rounded-lg ring-1 ring-slate-200"
              onMouseMove={onMove(i)} onTouchMove={onMove(i)}>
              <canvas ref={(el) => { canvasRefs.current[i] = el; }} className="rounded-lg" />
              {!ready && <div className="absolute inset-0 grid min-h-[220px] place-items-center bg-white/70"><Loader2 size={22} className="animate-spin text-[#F97316]"/></div>}
              {blocks.map((b) => pos[b.id] && (pos[b.id].page || 1) === i + 1 && (
                <div key={b.id} data-testid={`hrl-placed-${b.id}`}
                  onMouseDown={startDrag(b.id, i)} onTouchStart={startDrag(b.id, i)}
                  style={{ left: `${pos[b.id].x}%`, top: `${pos[b.id].y}%` }}
                  className="absolute cursor-grab rounded-lg border-2 border-dashed border-violet-500 bg-violet-50/90 px-2 py-1 shadow-md active:cursor-grabbing">
                  {b.signature_data_url
                    ? <img src={b.signature_data_url} alt="" className="h-8 max-w-[90px] object-contain" draggable={false} />
                    : <div className="h-6 w-20 border-b border-violet-400" />}
                  <div className="mt-0.5 flex items-center justify-between gap-1">
                    <span className="text-[8px] font-bold text-violet-700">{b.label || b.name || "Signer"}</span>
                    <button onMouseDown={(e) => e.stopPropagation()} onClick={() => setPos((p) => ({ ...p, [b.id]: null }))}
                      className="text-[8px] font-bold text-rose-500" data-testid={`hrl-unplace-${b.id}`}>✕</button>
                  </div>
                </div>
              ))}
            </div>
