...
const PROVIDERS = [
  ...
  { v: "gemini", label: "Google Gemini", desc: "Free tier available at aistudio.google.com",
    keyHint: "starts with AIza...", url: "https://aistudio.google.com/apikey" },
  { v: "emergent", label: "Emergent Universal Key", desc: "Works only on Emergent's preview/cloud",
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">
          {PROVIDERS.map((p) => (
            <button key={p.v}
              ...
              className={`text-left rounded-2xl border-2 p-3 transition ${form.provider === p.v ? "border-[#F97316] bg-orange-50" : "border-slate-200 hover:border-slate-300"}`}>
              <div className="font-bold text-[#0F2042]">{p.label}</div>
              ...
              {p.url && <a href={p.url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}
                className="mt-1 inline-block text-[10px] font-bold text-[#F97316] underline">Get key →</a>}
            </button>
          ))}
        </div>
        ...
        <div className="mt-5 space-y-3">
          <label className="block">
            <span className="mb-1 block text-[10px] uppercase tracking-[0.2em] text-slate-500">API key * ({provDef.keyHint})</span>
            ...
            <span className="mt-1 text-[10px] text-slate-500">Keys are stored encrypted at rest in the database. Never logged.</span>
          </label>
          <label className="block">
