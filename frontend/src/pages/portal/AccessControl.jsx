...
const MODULE_GROUPS = [
  ...
  { label: "Operations", mods: ["finance", "issues", "presence", "email-campaigns", "email-templates", "email", "calendly"] },
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 self-start rounded-full bg-gradient-to-r from-[#F97316] to-[#EA580C] px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-orange-200 active:scale-95"
        >
...
export function MatrixTab() {
  ...
  const load = async () => {
    try {
      ...
      setData(data); setMatrix(data.matrix); setDirty(false);
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.put("/rbac/matrix", { matrix });
      await refreshPermissions(); // bust the cache so the change is reflected immediately
      toast.success("Permissions saved — UI refreshed for all roles");
      setDirty(false);
