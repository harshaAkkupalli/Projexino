...
import {
  Bell, Search, LogOut, Home, ChevronLeft, ChevronRight, X, Menu,
} from "lucide-react";
...
// === Sibling-page navigation order (left/right chevrons) ===
const PORTAL_ORDER_FULL = [
  "dashboard", "projects", "tasks", "chat", "leads", "team",
  "manager", "badges", "documents", "issues", "finance",
  "email-campaigns", "email-templates", "presence", "ai", "ai-settings",
  "notifications-permissions", "doc-verification", "hr",
  "calendly",
  "profile", "settings",
];
const INTERN_ORDER_FULL = ["dashboard", "tasks", "documents", "badges", "chat", "ai", "profile"];
...
export default function AppShell() {
  ...
  // === Sibling navigation order (mobile swipe + chevron support) ===
  const PORTAL_ORDER = PORTAL_ORDER_FULL;
  const INTERN_ORDER = INTERN_ORDER_FULL;
  const ADMIN_ONLY = new Set(["settings", "notifications-permissions", "ai-settings"]);
  const PRIV_ONLY = new Set(["manager", "badges", "finance", "email-campaigns", "email-templates", "presence", "doc-verification", "hr", "calendly"]);
  const isAdminLike = (r) => r === "admin" || r === "super_admin";
  const order = useMemo(() => {
    const src = isIntern ? INTERN_ORDER : PORTAL_ORDER;
    if (!user) return src;
    return src.filter((s) => {
      if (ADMIN_ONLY.has(s) && !isAdminLike(user.role)) return false;
      if (PRIV_ONLY.has(s) && !["admin", "super_admin", "manager", "hr"].includes(user.role)) return false;
      return true;
    });
  }, [user, isIntern]);
  const currentIdx = order.indexOf(tail);
  const prevSlug = currentIdx > 0 ? order[currentIdx - 1] : null;
  const nextSlug = currentIdx >= 0 && currentIdx < order.length - 1 ? order[currentIdx + 1] : null;
  const goPrev = () => prevSlug && navigate(`${base}/${prevSlug}`);
  const goNext = () => nextSlug && navigate(`${base}/${nextSlug}`);

  // Manual swipe detection — mobile only. Skip the handler entirely on
  // pointer-fine devices (desktop with mouse / trackpad) to prevent stuck-scroll feel.
  ...
  const onTouchEnd = (e) => {
    ...
    // Require a clearly-horizontal, fast swipe AND a meaningful distance.
    if (Math.abs(dx) < 100 || Math.abs(dy) > Math.abs(dx) * 0.6 || dt > 600) return;
    if (dx > 0) {
      if (prevSlug) goPrev();
      else if (!isHome) navigate(base);
    } else if (nextSlug) {
      goNext();
    }
  };
    ...
    <div className="portal-scope min-h-screen app-shell-bg text-[#0F172A]">
      ...
      >
        ...
        <div className="app-main">
          ...
          {/* Page body — swipe on mobile inner pages, vertical scroll on desktop */}
          {isHome ? (
            <main className="app-page-launchpad">
              <Outlet />
            </main>
          ) : (
            <main
          ...
          {!isHome && (
            <>
              {prevSlug && (
                <button
                  data-testid="app-prev-btn"
                  onClick={goPrev}
                  aria-label={`Previous page: ${TITLE_MAP[prevSlug] || prevSlug}`}
                  className="app-page-chev app-page-chev-prev"
                >
                  <ChevronLeft size={18} />
                </button>
              )}
              {nextSlug && (
                <button
                  ...
                  onClick={goNext}
                  aria-label={`Next page: ${TITLE_MAP[nextSlug] || nextSlug}`}
                  className="app-page-chev app-page-chev-next"
                >
      ...
      <NotificationSheet open={notifsOpen} onClose={() => setNotifsOpen(false)} notifs={notifs} unread={unread} onMarkAll={markAllRead} />
...
function NotificationSheet({ open, onClose, notifs, unread, onMarkAll }) {
  return (
    <AnimatePresence>
      {open && (
        <>
